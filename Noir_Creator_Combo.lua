-- Noir_Creator — Overdrive H mega-plugin (Aimlock · BJP · PM VALEX · PrismFlux · Omega · Emotes)
-- Combined Overdrive H plugin — Noir_Creator
-- Modules: Aimlock, BindableButtonsColor, BJP, ButtonTransparency, PM_VALEX,
--           Pm-WallHop, PrismFlux, Shiftlock Crosshair, CFG/Omega, 7yd7 Emotes

-- ============================================================
-- SINGLE TAB MODE
-- All modules write into one plugin tab -> in the menu it looks like ONE plugin.
-- The tab name is set in the title field below.
-- Want separate tabs per module again? Set separateTabs = true.
-- ============================================================
local SR_UI = {
    separateTabs = false,                                      -- false = one shared tab for everything
    title        = "SteelRework",                              -- tab name in the plugin list
...
-- so paragraphs are built from plain labels: a title plus lines of ~58 characters.
...
-- ============================================================
-- SHARED ROTATION DRIVER
-- Every rotating button stroke/gradient used to own its own RenderStepped
-- connection, so N buttons meant N callbacks per frame. They all share this one
-- connection now: the rotation still advances once per frame, but the cost no
-- longer grows with the number of buttons.
--   SR_Rota.Attach(obj, maid, degreesPerSecond) -> handle
--   handle:Destroy() / handle:Disconnect() stop that one rotation again.
-- ============================================================
...
function SR_Rota.Attach(obj, maid, speed)
    ...
    function handle.Destroy()
        ...
        -- last one gone: stop the shared loop right away instead of burning frames
...
-- ============================================================
-- SETTINGS FILE STORE
-- Executors lie about writes: writefile may return false (or nothing) even when
-- the file was written, and on Android it can fail on scoped storage. So every
-- save is written and then READ BACK and compared, alternative writer/reader
-- names are tried, and the last good payload is also kept in memory — a reload
-- in the same session restores the settings even if nothing can be written.
-- ============================================================
-- ============================================================
-- SETTINGS FILE STORE
-- Executors lie about writes: writefile may return false (or nothing) even when
...
-- in the same session restores the settings even if nothing can be written.
--
-- A failed save is retried with every writer, then after deleting the (possibly
-- locked) file, then through a bare file name. If all of that fails, one console
-- line explains what the executor actually offers (see SR_Store.probe).
-- ============================================================
...
-- Looks for the file functions the executor gives us. Safe to call again later:
-- some executors inject them a little after the script has started.
function SR_Store.scanAliases()
    ...
    if env then
        ...
        -- the executor env table can be protected (like the host table): never assume
...
-- Runs once, only when a real save could not be written: it shows in the console
-- what the executor answers to a tiny test write, so the reason is visible.
...
-- Reads a file: alternative readers first, then the in-memory copy from this session.
function SR_Store.read(path)
    ...
    if not text and #SR_Store.R == 0 then
        SR_Store.ensureAliases()      -- the executor may hand the functions over later
        text = tryReaders()
    end
...
-- Writes a file and verifies it. Returns true on success, or false plus the reason.
-- Every writer is tried, then the same again after deleting the file (a stale or
-- locked file blocks the write on some executors), then the file is built in
-- pieces through appendfile, and finally isfile is asked whether it is there.
function SR_Store.write(path, text)
    if type(text) ~= "string" then return false, "payload is not a string" end
    SR_Store.ensureAliases()
    local mem = SR_Store.mem
    if mem then
        -- memory keeps working even if the disk does not (the table can be protected)
    ...
    -- is the file there now? (used when the executor gives no reader)
    ...
    local function attempt(fn, target)
        ...
        if result ~= false then return true end           -- no reader: trust a non-false result
        if canLook and exists(target) then return true end -- the file is there: the "false" was a lie
    ...
    for t = 1, #targets do
        ...
        drop(target)                                      -- a stale or locked file blocks writes
        for i = 1, #SR_Store.W do
            local ok, why = attempt(SR_Store.W[i], target)
            if ok then return true end
            lastError = tostring(SR_Store.WNames[i]) .. " after delete: " .. why
        end
        -- only appending works on some executors: build the file piece by piece
        ...
        -- nothing could confirm the write: if the file is there, accept it
...
-- ============================================================
-- BUTTON POSITIONS (Aimlock / BJP / Wallhop on-screen buttons)
-- Kept inside SR_Store on purpose: the main chunk is limited to 200 locals and
-- every new top-level local pushes the rest of the file over that limit.
-- The position of a button is saved the moment a drag ends, in one small file,
-- verified and retried by SR_Store.write (with an in-memory copy as fallback).
-- ============================================================
SR_Store.posFile = "Noir_Creator_button_positions.json"
SR_Store.pos = nil               -- { [module] = { [buttonId] = {xs,xo,ys,yo} } }
...
-- {xs, xo, ys, yo} of a saved button, or nil when nothing was saved yet.
...
-- Fire-and-forget write: verified by read-back, retried a few times on failure.
...
-- Console diagnostics only (never a toast): they show up in the executor console
-- as [Noir_Creator] [pos] ... and are capped so the log cannot flood.
...
-- Puts an already existing button back on its saved place. Used right after a
-- module has finished building its layout, so nothing can push it back.
...
-- Only ONE message is shown — the plugin load message (see SR_BootNotify
-- at the very end of the file). All other module toasts (module loading, colour
-- changes, hotkeys, config, errors etc.) are hidden but NOT lost:
-- they go to the executor console tagged [Noir_Creator].
--
-- SR_UI.notifyMode:
--   "bootOnly" — load message only (default)
--   "all"      — all notifications, like the original
--   "none"     — nothing at all, not even the load message
-- ============================================================
SR_UI.bootMessage = "Noir plugin load Successfully"
SR_UI.notifyMode  = "bootOnly"
SR_UI.bootSeconds = 12        -- quiet window after boot (only used in "all" mode)
SR_UI.notify      = odh_shared_plugins and odh_shared_plugins.Notify
-- Diagnostics instead of toasts: log to the console, not to the UI.
local function SR_Log(text)
    pcall(print, "[Noir_Creator] " .. tostring(text))
end
-- Whether toasts are shown right now.
local function SR_ToastsEnabled()
    if SR_UI.notifyMode == "all" then return not SR_UI.bootQuiet end
    return false
end
-- Single gate for ALL plugin notifications: false = hide (and log to the console).
...
if type(SR_UI.notify) == "function" then
    ...
    -- a __newindex-protected host table silently drops a plain write — use rawset
...
-- Called once at the very end of the file: the plugin's only notification.
local function SR_BootNotify()
    if SR_UI.notifyMode ~= "none" and type(SR_UI.notify) == "function" then
        pcall(SR_UI.notify, SR_UI.bootMessage, 6)      -- bypasses the gate: this is that very message
    end
...
-- ============================================================
-- Shared ODHX adapter factory (replaces per-plugin boilerplate)
-- ============================================================
local function CreateODHX(id, title, file, replay, external)
    ...
    local function report(message)
        ...
        -- console only: exactly one visible notification is allowed (the boot one)
        ...
        if SR_Store.CanRead() then
            ...
            if text then
                ...
                if good and type(data)=="table" and data.version==1 and type(data.controls)=="table" then
                    ...
                    -- main file is corrupted: try the backup copy before giving up
    ...
    -- Real write. The file is verified by reading it back (see SR_Store), so a
    -- lying executor cannot break saving. No extra .bak file is created.
    local function commitNow()
        ...
        local ok,err=pcall(function()
            ...
            elseif SR_Store.CanWrite() then
                ...
                -- no extra .bak file is created: the write is verified by reading it back
    ...
    X.Flush=commitNow
    -- Slider drags change a control dozens of times per second; writing the file on
    -- every event is both laggy and unsafe. The in-memory data updates immediately,
    -- the file is written 0.4 s after the last change, and Stop/Finish flush at once.
    function X.Commit()
        if not X.ready or X.silent or X.restoring or X.stopped or X.committing then return end
    ...
    function X.Finish()
        ...
        X.ready=true
        if not X.badFile then X.Flush() end      -- synchronous: the file exists right after load
    end
    ...
    function X.shared.AddSection(name,subtitle)
        ...
        -- in single-tab mode a card is tagged with the module it came from
    ...
    function X.Stop()
        ...
        X.Flush()                 -- write the current state synchronously before teardown
        X.stopped=true
        for _,c in ipairs(X.connections) do pcall(function() c:Disconnect() end) end
        X.connections={}
        -- Run the module's own cleanup: it flushes files and removes its GUI.
        -- Without this the module teardown (and its pending saves) never happened.
...
do
    ...
    -- Improved by Noir_Creator
    -- Developer: Noir_Creator | V4 Edition
                ...
                releaseConn = ODHX.Connect(UserInputService.InputEnded, function(endInput)
                    if endInput.UserInputType == input.UserInputType then
                        ...
                        if not hasMoved then
                            ...
                            SR_Store.posSave("aimlock", gui.Name, gui.Position)   -- remember the new place
                        end
    ...
    function BindableButtons.AddBButton(id, text, onFunc, offFunc)
        ...
        local savedPos = SR_Store.posGet("aimlock", id)      -- the place the button was left at
        if savedPos then
        ...
        -- Gradient rotation: one shared RenderStepped for the whole plugin
        -- (previously each button had its own connection = N callbacks per frame).
    ...
    ODHX.Finish()
    -- the layout is final now: put every button the user moved back on its place
    for btnId, btn in pairs(BindableButtons.Buttons) do SR_Store.posApply("aimlock", btnId, btn) end

end

-- ========================================
-- Module: Bindable Buttons Color (BindableButtonsColor)
-- ========================================
do
...
do
            ...
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                ...
                rel = ODHX.Connect(__UIS.InputEnded, function(endInput)
                    if endInput.UserInputType == input.UserInputType then
                        ...
                        else SR_Store.posSave("bjp", gui.Name, gui.Position) end   -- remember the new place
                        rel:Disconnect()
                    end
                end)
            end
    ...
    local function AddBigButton(id, text, func, isGold, customSize)
        ...
        local savedBBPos = SR_Store.posGet("bjp", id)        -- the place the button was left at
        if savedBBPos then
                ...
                rel = ODHX.Connect(__UIS.InputEnded, function(endInput)
                    if endInput.UserInputType == input.UserInputType then
                        ...
                        if not hasMoved then
                            ...
                            SR_Store.posSave("bjp", gui.Name, gui.Position)   -- remember the new place
                        end
                        rel:Disconnect()
                    end
                end)
    ...
    function BindableButtons.AddBButton(id, text, clickFunc, isGold, customSize)
        ...
        local savedPos = SR_Store.posGet("bjp", id)          -- the place the button was left at
        if savedPos then
    ...
    -- Extended bomb-name list for MM2
    local BOMB_NAMES = {
        "FakeBomb", 
        "Bomb", 
        "GiftBomb", 
        "PresentBomb",
        "Snowball",  -- For winter events
        "CandyBomb"  -- For Halloween events
    }

    -- Configuration for different bomb types
    local BOMB_CONFIGS = {
        FakeBomb = {
            Cooldown = 22,
            Power = 58,
    ...
    -- ============================================
    -- UNIVERSAL BOMB JUMP SYSTEM
    -- ============================================

    local function CreateBombJumpSystem(config)
        -- config = {
        --     bombType = "FakeBomb" or "GoldBomb",
        ...
        local function GetCenterPosition()
            ...
            if character and character:FindFirstChild("HumanoidRootPart") then
                ...
                -- Drop the bomb under our feet for maximum effect
        ...
        local function GetAnyBomb(bombName)
            ...
            -- Check every possible bomb name
            local bombNamesToCheck = {bombName}
            if bombName == "FakeBomb" then
                -- For a normal bomb check all possible names
            ...
            -- Try to get the bomb through the Remote with a timeout
            local success = false
            local attempts = 0
            ...
            if success and bomb then
                ...
                if position then
                    ...
                    if root then
                        ...
                        -- Add to the current velocity instead of replacing it
...
-- ========================================
-- Module: Button Transparency (ButtonTransparency)
-- ========================================
do
    local ODHX = CreateODHX("ButtonTransparency", "Button Transparency", "ODH_ButtonTransparency_settings.json", false, false)
    --[[ ============================================================
          Overdrive H  |  Plugin: Button Transparency (v6)
          ------------------------------------------------------------
          KEY IN v6 — PERIODIC FREEZES ARE GONE:
            • Fully event-driven architecture: NO regular
              UI-tree scans. Roblox itself reports new buttons
              through the DescendantAdded event — the plugin checks
              ONLY the new object (O(1)) instead of the whole tree.
              Previously thousands of CoreGui objects were scanned
              every ~6 seconds and EVERY node ran a recursive text
              lookup — that caused a freeze every 2-3 seconds.
            • The hot filter was rewritten: one pcall per object,
              ClassName compared via a table, and the recursive
              text search runs ONLY on small buttons (not every node).
            • The maintenance loop remains but only does cheap
              repainting of 2-3 cached buttons; a light check of the
              hub GUI every ~24 seconds (not the whole CoreGui).
            • Duplicate guard: if the plugin is loaded again,
              the old copy is unloaded (Shutdown) so two loops never
              run at the same time.
            • All functionality is kept: one slider
              "Button opacity", pick a button by tap, remove the
              manual pick, hard reset. Opacity is additive
              (the colour never changes), 0 = as before, 100 = invisible.

          IMPORTANT: after loading v6 rejoin the game once —
          so that the old v5 copy (its loop cannot be stopped from outside)
          is guaranteed to unload.

          Install: like PluginExample — load the file through the
          Overdrive H menu as a plugin.
        ============================================================ ]]
    ...
    local DEFAULT_VALUE      = 0   -- starting opacity, % (0 = as before)
    local APPLY_INTERVAL     = 4   -- loop period (cheap cache repaint only)
    local LIGHT_RESCAN_TICKS = 6   -- light check of the hub GUI every N ticks (~24 s)
    local BIG_MIN_WIDTH      = 100 -- wider (px) = large panel
    local PICK_SECONDS       = 6   -- how many seconds we wait for the pick tap
    ...
    local function genAlive()
    	if MY_GEN == 0 then
    		return true -- getgenv unavailable, no one to compete with
    	end
    	local ok, res = pcall(function()
    ...
    --========================= PLUGIN STATE =========================--
    local State = {
    	Value      = DEFAULT_VALUE, -- opacity, %
    	Active     = false,         -- slider touched? (false after reset)
    	ManualBind = nil,           -- button picked by tap
    	Dead       = false,         -- instance unloaded by a newer copy
    }
    ...
    -- cache of found buttons; painting works from the cache only — fast
    local Cache = {
    	list  = {},
    	dirty = true,
    	tick  = 0,
    ...
    -- forward declarations (defined below, addCandidate needs them earlier)
    ...
    -- are all ancestors visible? (called ONLY for matched candidates)
    local function isVisible(inst)
    	local cur = inst
    	local depth = 0
    	while cur and depth < 60 do
    ...
    -- is the object inside the Overdrive H window/menu itself
    -- (matched candidates only — not on the hot path)
    local function isInsideOverdriveMenu(inst)
    	local cur = inst.Parent
    	local depth = 0
    	while cur and depth < 60 do
    ...
    -- is the object inside SYSTEM roblox-gui (RobloxGui and
    -- similar) — such interfaces are never touched
    local function insideRobloxGui(inst)
    	local cur = inst.Parent
    	local depth = 0
    	while cur and depth < 60 do
    ...
    --================= FAST CANDIDATE FILTER (hot path) ================--
    -- One pcall, minimal allocations. A recursive text search runs
    -- only for buttons (they have few children). Returns true/false.
    local function probeCandidate(inst)
    	local ok, res = pcall(function()
    		local cls = inst.ClassName
    		local isBtn  = BTN_CLASSES[cls] or false
    		...
    		-- recursive text search ONLY on buttons (small subtree)
    		local childText = nil
    		if isBtn then
    			local lbl = inst:FindFirstChildWhichIsA("TextLabel", true)
    			if lbl then
    ...
    -- small button with a shoot/murderer/bind keyword
    ...
    -- full scan (ONLY on load / first slider touch)
    local function fullScan()
    	...
    	-- merge with the current tap pick
    	for _, c in ipairs(Cache.list) do
    ...
    -- light check: the hub ScreenGui only (not the whole CoreGui),
    -- catches buttons whose text was set AFTER creation
    local function lightScan()
        local restored=ODHX.Resolve(State.ManualPath)
    ...
    -- Result = 1 - (1 - original) * (1 - given): the colour is unchanged.
    ...
    -- FAST painting: cache only, NO scans. Called by the slider
    -- and the loop — costs next to nothing.
    local function applyCached()
    	if not State.Active then
    		return 0
    	end
    ...
    local function onDescendantAdded(inst)
    	...
    	-- O(1) check of the NEW object only
    	local okG, isGui = pcall(function()
    		return inst:IsA("GuiObject")
    	end)
    	if not (okG and isGui) then
    ...
    --==================== MAINTENANCE LOOP (LEAN) ====================--
    -- Cheap cache repainting only; a light check of the hub GUI —
    -- every LIGHT_RESCAN_TICKS ticks. NO full scans.
    local eventsOk = false
    pcall(function()
    	eventsOk = connectEvents()
    end)
    ...
    -- instance unload (called by a newer copy on reload)
    local function shutdown()
    	State.Dead = true
    	disconnectEvents()
    	restoreAll()
    ...
    -- debug access for the executor console (NOT via shared!)
    pcall(function()
    	if type(getgenv) == "function" then
    ...
    section:AddSlider("Button opacity", 0, 100, State.Value, function(v)
    	...
    	State.Active = true
    	-- synchronous full scan ONLY if the cache is empty and stale
    	-- (after a reset). Normal dragging performs no scans.
    	if Cache.dirty and #Cache.list == 0 then
    		Cache.dirty = false
    		pcall(fullScan)
    	end
    ...
    -- the first scan is deferred so plugin loading is not slowed down
    later(1, function()
    	if not State.Dead then
    		pcall(fullScan)
    		pcall(applyCached)
...
do
    ...
    --[[
        ...
        Author: Noir_Creator
        ==========================================================================
        Targeted and mass void-reset plugin for MM2: reset by role
        (Sheriff / Murderer), by list, by nearest target, auto modes, aura,
        click-to-reset, hotkeys, floating binds and a live HUD status.

        ── FEATURES ───────────────────────────────────────────────────────────────
          • Manual reset: Sheriff, Murderer, Everyone, Selected, Nearest, Cancel
            plus picking a specific player from a list.
          • Auto modes: Auto Sheriff, Auto Murderer, Loop (over the list),
            Aura (by radius), Click (reset by clicking a player).
          • Floating bind buttons: draggable with saved position, size,
            activity highlight, shared click sound.
          • Hotkeys for every action: capture a key with a toggle, ignore
            modifiers and keys typed while the chat is open.
          • HUD status: pulse, current target name, dragging with saved position.
          • Lists: target pick, Loop list, whitelist (add/remove/clear).
          • Settings saved to a file (readfile/writefile + JSON): timings,
            button/HUD sizes and positions, hotkeys, whitelist; debounced autosave.
          • "🛑 Panic" stops everything at once; "🧹 Clean duplicates" clears
            foreign and outdated menu sections.

        ── ARCHITECTURE AND RELIABILITY ───────────────────────────────────────────
          • One shared RenderStepped dispatcher for the whole plugin instead of N connections;
            hover/press animations are computed by hand, without Tween objects.
          • Shared input handlers for the binds (1x InputChanged, 1x InputEnded),
            the drag state is a plain record: no connection leaks.
          • Multitouch: drag and click react only to the finger that pressed the button.
            A thumb on the movement joystick never drags or presses it.
          • The role remote is found once and cached (TTL + invalidation when
            players change); target search uses squared distances, no sqrt.
          • The bind layout is recomputed on add/remove/size change —
            buttons never drift or overlap.
          • FallenPartsDestroyHeight is always restored: cleanup + watchdog.
          • "⏹ Cancel" stops the current reset, the mass one and any retry.
          • An auto mode turns off instantly: the thread is killed, not left to finish a wait.
          • Dead targets are never reset; a per-target cooldown prevents spam.
          • A unique marker on OUR GUI cards: auto-cleanup can never touch
            foreign sections or the host window, and unload removes strictly our own.
          • Unload is available to the host and to a rerun: a repeated load first
            cleanly unloads the previous instance and only then builds the new GUI.
          • Headless mode: if the host has no AddSection — the core, HUD, binds and hotkeys
            keep working; all host API calls are wrapped in protection.

        ── SETTINGS ───────────────────────────────────────────────────────────────
          Config: pm_valex.json (in the executor working folder)
          Renaming the plugin: the BRAND / AUTHOR / PLUGIN_ID constants below.
    ]]

    -- ── Plugin identity (changed in one place) ─────────────────────────
    local AUTHOR            = "Noir_Creator"
    local BRAND             = "PM VALEX"            -- displayed name
    local PLUGIN_ID         = "pm_valex"            -- internal names: files, GUI, markers
    local PLUGIN_NAME       = BRAND .. " • VOID RESET"
    local VERSION           = "V1.1"
    local VERSION_TAG       = "pmvalex"             -- unique marker in the menu
    local MARKER_PREFIX     = "@pmvalex_"           -- marker prefix of "our" card
    local CONFIG_PATH       = PLUGIN_ID .. ".json"
    local STORAGE_NAME      = "@" .. PLUGIN_ID
    local UNLOAD_GLOBAL     = "__PM_VALEX_UNLOAD"
    -- GUI storages of foreign/old builds to remove on start
    local LEGACY_STORAGES   = { "@bindstorage_v6", "@bindstorage_v5" }
    -- clean up sections of old menu builds (LEGACY_TITLES list below)
    local CLEAN_LEGACY_MENU = true

    -- ====== Host notifications (work without shared.Notify too) ======
    -- StarterGui is declared early and filled in the services block below:
    -- hostNotify is called from the guard, i.e. before services are initialised
    ...
    -- ====== Reload: unload the previous instance first ======
    -- Important: do this BEFORE creating our GUI, otherwise the new instance picks up
    -- the old ScreenGui and its cleanup then destroys our interface.
    pcall(function()
        if type(getgenv) ~= "function" then return end
        local g = getgenv()
        if type(g) ~= "table" then return end
        local prev = rawget(g, UNLOAD_GLOBAL)
        rawset(g, UNLOAD_GLOBAL, nil)   -- clear right away so it is not called twice
    ...
    -- ====== Configuration ======
    -- references are filled below but declared here: serializeConfig/applyLoaded
    -- use them as upvalues rather than as globals
    ...
    local DEFAULTS = {
        ...
        auraInterval      = 0.4,
        bindButtonSize    = 0.11,
        targetCooldown    = 0.8,   -- new: min pause between resets of the same target
        roleCacheTTL      = 0.8,   -- new: role cache lifetime
        muteSounds        = false, -- was: local muteButtonSounds
        notifications     = true,  -- new
        keybinds          = {},    -- new: {sheriff="F", ...}
        bindPositions     = {},    -- new: {[id]={x=,y=}}
        binds             = {},    -- new: {[actionId]=true} — which on-screen bind buttons are on
        hudPos            = { x = 0.5, y = 6 },
        pluginUI          = {}, -- persistent toggle/slider state in the existing config
        whitelist         = {},    -- new: {{id=,name=}, ...}
    }
    ...
    -- whitelist: {[UserId]=Name} in memory <-> {{id,name}} list in the file
    ...
    -- A failed write is usually temporary (file busy, boot-time hiccup), so the
    -- save is repeated a few times instead of being lost.
    ...
    -- ====== Positions (binds + HUD) in their own tiny file ======
    -- The button layout is what the user notices first when it is lost, so it is
    -- kept separately too: even if the big config write fails, the layout survives.
    -- A failed write is retried a few times, and SR_Store keeps a copy in memory,
    -- so even a reload in the same session restores the positions.
    ...
    ODHX.backend=function(data)
        ...
        return saveConfig()          -- debounced: the file is written once the changes settle
    end

    -- ====== State ======
    local state = {
        whitelist       = loadedWhitelist,   -- {[UserId] = Name}
        selectedPlayers = {},                -- array for Loop
        selectedSet     = {},                -- {[UserId] = true} — fast dedup
        resetSelPlr     = nil,
        lastResetAt     = {},                -- {[UserId] = os.clock()} — target cooldown
    }
    ...
    -- ====== Single ticker (1 RenderStepped for the whole plugin) ======
    ...
    -- ====== Section titles: legacy (clean up) / current (leave alone) ======
    -- LEGACY_TITLES feeds the auto-cleanup of foreign menu junk: the scanner finds
    -- and removes old-build sections by these strings. Not needed? CLEAN_LEGACY_MENU = false.
    local LEGACY_TITLES = {
        ["⚡ Quick Actions"] = true,
        ["🤖 Automation"] = true,
        ...
        ["📊 Status"] = true,
        ["ℹ️ Info"] = true,
        ["⚡ Reset"] = true,             -- matching names are caught by the marker
        ["🤖 Auto"] = true,
        ["📋 Lists"] = true,
        ["⚙️ Tuning"] = true,
        ["⚙ Tuning"] = true,
    ...
    -- Add titles of any old sections to clean up here:
    --   EXTRA_LEGACY_TITLES = { ["Old section title"] = true }
    ...
    -- guard words: a card containing them is the host window and must not be touched.
    -- IMPORTANT: VERSION_TAG is not included, otherwise we could not find and tag
    -- our own main card (the version check lives in cardIsOurs).
    local HOST_WORDS = {
    ...
    local function getStorage()
        ...
        -- clean storages of previous builds if any are left
        pcall(function()
    ...
    -- protection against missing/failing methods of the host menu
    -- The host invokes control callbacks while still creating them. That used to
    -- wipe saved hotkeys and toggles (the callback got 'false') and sent a burst of
    -- "Hotkey: ... key cleared" toasts. Callbacks stay blocked until the UI is built.
    local pmReady = false
            ...
            __index = function(p, key)
                ...
                if type(value) == "function" then
                    return function(first, ...)
                        -- a call like sec:AddButton(...) passes the proxy as the first
                        -- argument — it must be dropped, otherwise every argument
                        -- shifts by one position
                        local args = table.pack(...)
                        if first == p then
                            -- (arguments no longer include the proxy)
                            ...
                            -- the proxy did not arrive first: shift everything back
    ...
    -- ====== GUI scanner: card lookup, markers, deduplication ======
    ...
    -- memoised "is this the host window?" check (was: recounted for every label)
    ...
    -- walks up from a label looking for the section "card"
    ...
    -- Chunked scanning. GetDescendants() over CoreGui/PlayerGui is tens of thousands
    -- of instances, so every SCAN_CHUNK instances we yield a frame (task.wait()).
    -- Declared here, before its first use: the purge below already needs it.
    ...
    -- forward declaration: the body is assigned below, once findCard/attachMarker exist
    local markOwnCards

    -- "our" card: it carries our marker OR shows our version text
    -- (fallback in case the marker was not attached)
    ...
    -- removes cards not created by this run (old versions + foreign duplicates)
    ...
    -- Optimisation: we keep a DescendantAdded subscription on the GUI roots and run
    -- the heavy scan (GetDescendants over CoreGui/PlayerGui) only when the menu changed.
    local guiDirty = true
    local dirtyWatches = {}
    ...
    local function scanIfDirty()
        ...
        guiDirty = false
        pcall(markOwnCards)      -- mark our own cards first, then purge foreign ones
        pcall(purgeForeign, true)
        return true
    end

    -- tags our cards with a unique marker (after the menu is built)
    local markedCount = 0
    ...
    -- fallback if markers were not attached (old title-based logic)
    ...
    -- cut old-version junk before building our own menu
    pcall(purgeForeign, true)

    -- ====== Click sound (one for all buttons) ======
    ...
    -- ====== Bindable Buttons ======
    local BindableButtons = {
        Buttons = {},   -- {[id] = ImageButton}   (V5.x compatible)
        Maids   = {},   -- {[id] = Maid}          (V5.x compatible)
        recs    = {},   -- {[id] = animation/position record}
        order   = {},   -- layout order
        Count   = 0,
        ResetActive = false,
    ...
    -- grid layout + positions restored from config
    ...
    -- one shared move/release handler for all buttons
    local dragState = nil

    local binderGlobalMaid = Maid.new()
    RootMaid:GiveTask(binderGlobalMaid)

    -- A drag must react ONLY to the input that started it.
    -- On mobile every touch is a separate InputObject: accepting any
    -- Touch makes the thumb on the movement joystick drag the button along
    -- (exactly what happened in the first rewrite — the old plugin compared
    -- input == dragInput, so the button stayed in place there).
    local function isDragInput(input)
        if not dragState then return false end
        if input == dragState.dragInput then return true end   -- mouse: MouseMovement object of the button itself
        if input == dragState.input then return true end       -- touch: one InputObject per touch
        return false
    end
    ...
    binderGlobalMaid:GiveTask(UserInputService.InputChanged:Connect(function(input)
        ...
        if not isDragInput(input) then return end             -- foreign finger/cursor — ignore
    ...
    binderGlobalMaid:GiveTask(UserInputService.InputEnded:Connect(function(input)
        ...
        -- the press animation is always reset
        for _, rec in pairs(BindableButtons.recs) do rec.targetPress = 0 end
        if not dragState then return end
        -- A drag ends only when the SAME input that started it is released.
        -- Otherwise releasing the joystick thumb counted as a button tap
        -- and fired the action. For the mouse the InputObject on release may
        -- differ, so there we compare the button type (as in the old version).
        ...
        if not moved then
            ...
            saveConfig(true)          -- immediate: dragging is rare, and the position must survive a quick reload
            savePositions()           -- and also into the small positions file
        end
    end))

    -- one frame for all buttons (instead of N RenderStepped connections)
    local function binderStep(dt)
        ...
        for _, rec in BindableButtons.recs do      -- generalised iteration: Luau fast path
            ...
            if btn and glow then
                ...
                -- the glow follows the button; the button moves only while dragging
                ...
                rec.stroke.Rotation = rec.rot       -- changes every frame by design
            end
        end
    end
    ...
    function BindableButtons.AddBButton(id, text, clickFunc, isGold)
        ...
        -- the initial size is set right away so the button is not
        -- stretched before the first frame (binderStep recomputes it every frame)
        local camera = Workspace.CurrentCamera
        local screen = (camera and camera.ViewportSize) or FALLBACK_VIEWPORT
        ...
        -- drag + ripple + press in one connection
        buttonMaid:GiveTask(ImageButton.InputBegan:Connect(function(input)
            if input.UserInputType ~= MOUSE1 and input.UserInputType ~= TOUCH then return end
            rec.targetPress = 1
            dragState = {
                rec = rec,
                input = input,                            -- this exact finger/mouse button
                dragInput = nil,                          -- for mouse it is filled from InputChanged
        ...
        -- For the mouse the movement event arrives as a separate InputObject (MouseMovement),
        -- so we store it here — the global handler compares exactly this one.
        -- For touch it is the same object as in InputBegan.
    ...
    function StatusHUD.Init()
        ...
        RootMaid:GiveTask(UserInputService.InputEnded:Connect(function(input)
            if input.UserInputType ~= MOUSE1 and input.UserInputType ~= TOUCH then return end
            if hudDrag and hudDrag.moved then saveConfig(true); savePositions() end  -- immediate: HUD position must survive a reload
            hudDrag = nil
        end))

        -- pulse: updated ~30 times/s instead of every frame (saves work)
    ...
    -- ====== Role cache (the remote is looked up once) ======
    local roleRemote = nil
    local roleRemoteTried = -1e9   -- not 0: otherwise the first lookup is throttled while os.clock() < 5
    ...
    local function getCachedRoleData()
        ...
        roleCache.timestamp = t   -- do not hammer the server on error
    ...
    local function isValidTarget(player)
        ...
        if hum and hum.Health <= 0 then return false end   -- do not reset dead players
        return true
    end
    ...
    local function findMurderer()
        ...
        -- fallback when the remote is unavailable: look for a knife in the hands
        local players = Players:GetPlayers()
        for i = 1, #players do
            local player = players[i]
            if player ~= LocalPlayer and not state.whitelist[player.UserId] then
    ...
    -- body parts only (CanCollide used to be restored on accessories too)
    ...
    local function cancelCurrentReset()
        ...
        -- cancel the scheduled retry too, otherwise "Cancel" did not stop repeats
    ...
    local function VoidReset(TargetPlayer, _retryCount)
        ...
        -- per-target cooldown (does not interfere with retries)
        ...
        -- the target really resets — only now set the cooldown mark
        ...
        -- safety net: even if a frame "hangs", the state is restored
    ...
    -- ====== Auto modes (one implementation for all) ======
    local function startAutoModule(maidKey, finderFunc, intervalFn)
        ...
        maid:GiveTask(thread)   -- on disable the thread stops immediately, not after wait
        return maid
    end
    ...
    local function stopAutoModule(maidKey)
        if maids[maidKey] then
            maids[maidKey]:Destroy()
            maids[maidKey] = nil
            -- also kill the reset this mode started, together with its retries:
            -- otherwise a disabled auto mode kept finishing the target off
            cancelCurrentReset()
        end
    end

    -- ====== Actions (single source for menu, binds and hotkeys) ======
    local ACTIONS = {}
    ...
    autoSection:AddToggle("Aura", function(enabled)
        ...
        startAutoModule("resetAura", function()
            ...
            local maxDistSq = (config.auraStuds or 15) ^ 2   -- no sqrt
    ...
    autoSection:AddToggle("Click", function(enabled)
        ...
        local function resolveTarget()
            -- modern raycast first, GetMouse only as a fallback
            local char = LocalPlayer.Character
    ...
    listSection:AddPlayerDropdown("🛡 Un-whitelist", function(p)   -- new
    ...
    local function toggleBindButton(actionId)
        return function(enabled)
            ...
            -- remembered so the buttons come back by themselves after a rejoin
    ...
    -- ====== Tag our cards + repeated cleanups ======
    markOwnCards()

    RootMaid:GiveTask(task.spawn(function()
        -- Same passes, but the scan runs only when the GUI changed (see scanIfDirty):
        -- previously that meant 8 full CoreGui/PlayerGui walks on every launch.
    ...
    -- ====== Cleanup on unload ======
    RootMaid:GiveTask(function()
        -- Flush the pending debounced save FIRST: changes made in the last 0.35 s
        -- (dragged bind positions, HUD position) used to be dropped here.
        ...
        -- Now block writes: otherwise a late debounced save could store a cleared whitelist
    ...
    -- ====== Unload entry point ======
    -- In V5.2 the cleanup task sat in RootMaid but DoCleaning() was never called,
    -- so unload never happened. Now the function is available to the host and to
    -- a rerun (a repeated load unloads the previous instance
    -- at the very start, before building the new interface).
    ...
    ODHX.cleanup=unload
    ODHX.Finish()
    pmReady = true      -- UI is built: control callbacks work again

    -- ====== On-screen bind buttons come back after a rejoin ======
    -- While the menu is built, restored toggles are applied silently (see pmReady
    -- above): a "Bind ..." switch could look ON while its button was gone until
    -- the user flipped it again. This pass builds exactly the buttons that were
    -- enabled before the reload, removes the ones that were off, and then puts
    -- every button back on its saved place.
    local function reconcileBindButtons()
        for _, actionId in ipairs(ACTION_ORDER) do
            ...
            if not want then
                -- the toggle value restored from the settings file is the truth
...
do
    ...
    -- Create the section for our plugin
    ...
    -- Sensitivity setting (wall detection distance)
    ...
    -- === Floating button (like in Aimlock) ===
                ...
                releaseConn = ODHX.Connect(SR_UI.service("UserInputService").InputEnded, function(endInput)
                    if endInput.UserInputType == input.UserInputType then
                        ...
                        if not hasMoved then
                            ...
                            SR_Store.posSave("wallhop", gui.Name, gui.Position)  -- remember the new place
                        end
    ...
    function WallhopBindableButtons.AddBButton(id, text, onFunc, offFunc)
        ...
        ImageButton.Size = UDim2.new(widthScale, 0, buttonSizeY, 0)
        ImageButton.Position = UDim2.new(xPos, 0, yPos, 0)
        local savedPos = SR_Store.posGet("wallhop", id)      -- the place the button was left at
        if savedPos then
            ImageButton.Position = UDim2.new(savedPos.xs, savedPos.xo, savedPos.ys, savedPos.yo)
        end
        ImageButton.AnchorPoint = Vector2.new(0.5, 0.5)
    ...
    local function isWall(instance)
        ...
        -- Walk up the parents to check for a player owner
        local current = instance
        while current do
        ...
        -- Make sure it is a part with collision
    ...
    -- --- Raycast result helper for walls ---
    local function getWallRaycastResult()
        ...
        -- Blacklist other players' characters
        local players = Players:GetPlayers()
    ...
    performWallhop = function()
        ...
        -- Turn the player toward the wall
    ...
    -- --- Wall-seam detection (video fling) ---
    local lastHitInstance = nil
    local currentHitInstance = nil

    -- The character parts are cached per character (FindFirstChild ran twice on every
    -- frame before) and the raycast parameters are reused. The search is redone as soon
    -- as a part is missing, so behaviour stays the same during loading and respawns.
    ...
    ODHX.Connect(RunService.Heartbeat, function()
        ...
        -- Check whether space is held
        if not isJumpKeyPressed then 
            lastHitInstance = nil
            return 
        end

        -- Create a raycast with improved filtering (the params object is reused)
        ...
        -- Cast the ray along the camera direction
    ...
    -- --- Automatic Wallhop on jump (only when enabled) ---
    ...
    ODHX.Connect(UserInputService.InputEnded, function(input, gameProcessed)
        ...
        if input.KeyCode == Enum.KeyCode.Space then
            ...
            -- Reset detection when space is released
            lastHitInstance = nil
        end
    end)
    ...
    -- --- Extra: reset when focus is lost ---
    ODHX.Connect(UserInputService.WindowFocused, function()
        -- If the window lost focus, reset the jump state
        isJumpKeyPressed = false
        lastHitInstance = nil
    end)
...
do
    ...
    -- element count adjusted for quality (never below minimum)
    local function q(n, minimum)
    	return math.max(minimum or 1, math.floor(n * QUALITY + 0.5))
    end

    -- Texture library.
    --  • rbxasset://  — built into the client (load instantly, work in any game)
    --  • rbxassetid:// — official Roblox assets from Creator Docs tutorials
    --    (Mansion of Wonder, Volcano/Waterfall tutorials) — publicly available
    local TEX = {
    ...
    -- settings container: every module puts its block here via defineModule
    local S = {}
    ...
    -- k > 0 pulls the colour to white, k < 0 to black
    local function tint(c, k)
    	if k >= 0 then return c:Lerp(WHITE, math.min(k, 1)) end
    	return c:Lerp(BLACK, math.min(-k, 1))
    end

    -- ColorSequence / NumberSequence from a value list (even keys)
    -- v5.1 LIVELINESS: organic noise (a sum of incommensurable sines).
    -- Motion never loops: every effect (seed) has its own tempo character.
    ...
    -- v5.1: cseq never crashes on ready-made ColorSequences (expands to colour)
    local function cseq(...)
    	local list = { ... }
    ...
    -- palette from a single colour (or a Palette triple, or a rainbow hue)
    local function makePalette(cfg, hue)
    	...
    	p.seq      = cseq(bright, base, deep)      -- light → dark
    	p.seqUp    = cseq(deep, base, bright)      -- dark → light
    	p.seqHot   = cseq(WHITE, bright, base)     -- white core
    ...
    -- hue shift for the rainbow "on the fly" (white/black untouched)
    ...
    -- world-space point: an attachment on Terrain (Terrain sits at the origin)
    ...
    local function applyProps(inst, props)
    	...
    	for k, v in pairs(props) do
    		if k ~= "Parent" then
    			-- v5.1: Color3 for Color (ParticleEmitter/Beam/Trail) wraps itself
    ...
    -- Arc/ring built from N cubic Beziers. Span = fraction of the circle (1 = closed ring).
    ...
    -- Truncated cone from panels: rTop = 0 = cone, rTop = rBot = cylinder, height = 0 = disc.
    ...
    -- Wing feather: a ribbon from root to tip; spread = rise, sweep = backward angle, bend = curve
    ...
    -- Lightning: K jittering segments. bolt:set(p0, p1, jitter, seed) — coordinates in the parent space
    ...
    -- expanding arc/ring (Span < 1 = open arc)
    ...
    -- arc fan: N short arcs fly out from the centre, each tilting outward
    ...
    -- light pillar: a camera-facing ribbon growing upward; Twist > 0 = two braided ribbons
    ...
    -- fan of straight rays from a point
    ...
    -- shards: short ribbons fly out ballistically and spin
    ...
    -- expanding dome (a hemisphere made of a conical segment)
    ...
    -- lightning strike from the sky into a point
    ...
    -- radial particle fan (k emitters in a circle, outward or inward)
    ...
    -- camera-facing sprite: flash / star / sparkle with a size "pop"
    ...
    -- flipbook 8x8 explosion (OneShot: the whole frame grid plays within the particle lifetime).
    -- If the client has no Flipbook support, fall back to a static explosion-core texture.
    ...
    -- layered explosion: core -> halo -> sparks (Drag + gravity, stretched along velocity via Squash) -> smoke
    local function burst(cf, pal, o)
    	...
    	-- v5.2 DETAILS: ground shockwave + scattered sparkles on the ground
    ...
    -- water ripples (recipe from the waterfall tutorial): flat rings that grow and fade
    ...
    local PFPrim = {} -- shape kit (one outer table so the chunk keeps few locals)
    do

    -- shape point set inside a unit circle; returns a LIST of subpaths (polylines)
    local function shapePaths(kind, seed)
    	...
    	elseif kind == "flake" then
    		-- snowflake: 6 rays with two barbs each
    		for arm = 0, 5 do
    	...
    	elseif kind == "gear" then
    		-- gear: alternating outer/inner radius, 8 teeth
    		for i = 0, 32 do
    			local a = i / 32 * TAU
    ...
    -- animated ray figure: draw-on, spin, rise, pulse, fade
    local function polyFX(cf, o)
    	...
    	local draw = o.Draw or 0.45      -- fraction of dur used for drawing
    	local spin = o.Spin or 0
    	local rise = o.Rise or 0
    	local a0 = o.A0 or 0
    	local glow = (o.Glow ~= false)
    	local wall = (o.Plane == "wall") -- vertical plane (perpendicular to the cf view)
    	...
    	-- v5.3 CONSTRUCTOR: the figure's vertices assemble from parts flying in
    	local asms
    	if o.Assembly then
    		asms = {}
    		local seen = {}
    	...
    	Timeline.add(dur, function(a, dt, t)
    		...
    		-- v5.3 CONSTRUCTOR: parts fly into the vertices and live on them
    		if asms then
    ...
    -- ground cracks: jagged radial branches + dust + a dark blot
    ...
    -- "needle" spikes from the ground: conical rays GROW outward (they do not fly apart)
    ...
    -- comet: a glowing core flies along an arc from p0 to p1, with a trail and an impact burst
    ...
    -- ink blot: a dark spot + tentacle strands + splashes across the plane
    ...
    -- "matrix" rain: falling glyph strokes inside a cylinder
    ...
    -- lightning web: discharges between ring points + a flash in the centre
    ...
    -- pulsing heart outline + rising mini hearts
    ...
    -- RGB glitch: horizontal slices in three colours jitter and fade
    ...
    -- rune circle: a spinning swirl + strokes + a double hexagon
    ...
    -- crystal snowflake: a 6-ray frame grows and sparkles
    ...
    -- fire wings: two silhouette beats made of flame layers
    ...
    -- prism: a white core -> a fan of coloured rays + rainbow rings
    ...
    -- clockwork: dial segments, two hands, ticks
    local function clockFX(cf, o)
    	...
    	-- ticks: short flashes at 12 and 6
    	for k = 1, 4 do
    ...
    -- rising "wisp" souls with shadow eyes
    local function soulFX(cf, o)
    	...
    	-- v5.3 CONSTRUCTOR: a persistent assembly — parts fly into a 3D figure
    	-- and live on it (micro-orbit + flicker, no mechanical repeats).
    	-- kinds: ring / oval / sphere / cage / box / heart; returns (list, update)
    ...
    -- one Heartbeat for all persistent visuals. Tick.spin/wobble is the shared idle spin,
    -- Tick.cap limits the animation rate (Performance section).
    ...
    -- current rainbow hue per target (filled by the preset system)
    local RainbowHue = {}
    ...
    -- Module registry. A module = S[key] settings + instances + repainters + build/stop.
    -- Persistent modules rebuild automatically on respawn/quality change.
    -- v5.2 DETAILS: a second layer of persistent effects — flickering spark-moths around
    -- the carrier with breathing density. Lives in PFPrim to keep the locals small.
    do
    	local detailSeq = 0
    ...
    local function defineModule(key, defaults, opts)
    	...
    	-- rainbow without a rebuild: hue-shift every Beam/Particle/Trail/Light, remembering original colours
    	...
    	-- standard stop: drop the tick and destroy instances (a module may override it)
    	function m.stop()
    		Tick.drop(key)
    		m:clear()
    	end
    ...
    local function jumpStyle(style, cf, pal, size, dur, sparks)
    	if style == "Ripple" then
    		-- water: three concentric rings + a droplet "crown" + a glossy surface
    		for i = 1, 3 do
    	...
    	elseif style == "Twin Arc" then
    		-- two crescent slashes crossed + a spark at the crossing point
    		for i = 0, 1 do
    	...
    	elseif style == "Spiral Lift" then
    		-- a double helix carries upward + motes drawn into the centre
    	...
    	elseif style == "Shard Burst" then
    		-- crystals GROW out of the ground in a fan and melt + glass dust
    	...
    	elseif style == "Beacon" then
    		-- beacon pillar: a column core + rising rings + a beacon on top
    	...
    	elseif style == "Dome" then
    		-- flash dome + a hexagonal seal on the ground + dust inside
    	...
    	elseif style == "Meteor Ring" then
    		-- meteors fall in a circle INWARD and explode in the centre
    		local n = q(6, 4)
    		for i = 1, n do
    	...
    	elseif style == "Thunder Step" then
    		-- a rift + a web of discharges + ozone sparks
    	...
    	elseif style == "Bloom" then
    		-- flower: 6 petals draw on, the core flashes, pollen flies
    	...
    	elseif style == "Crescent" then
    		-- crescent moon: two offset rings form the moon + a vertical slash
    	...
    	elseif style == "Gear" then
    		-- gear: the toothed outline spins, the hub flashes, steam leaves the slots
    	...
    	elseif style == "Glass Break" then
    		-- shattered glass: cracks + shard needles + rainbow highlights
    	...
    	elseif style == "Ink Drop" then
    		-- blot: the spot creeps out in strands, splashes, hazy smoke
    	...
    	elseif style == "Star Sigil" then
    		-- a pentagram draws on + a counter star + runes on the vertices
    	...
    	elseif style == "Pulse Grid" then
    		-- cyber grid: lines draw on from the centre, nodes flicker
    	...
    	elseif style == "Phoenix" then
    		-- phoenix: fire wing beats + a flame pillar + a rain of embers
    	...
    	elseif style == "Nova" then
    		-- compact nova: flipbook explosion + shockwave + layered burst
    	...
    	elseif style == "Splash" then
    		-- splash: a water crown + ballistic droplets + ripples
    	...
    	elseif style == "Stardust" then
    		-- stardust: a swarm of star sprites spins into a vortex
    		...
    		-- smoke ring torus: puffy puffs fly out and melt + a shadow underlay
    ...
    -- mid-air jump (double jump in games that have it): a small ring under the feet in the air
    function Jump.air()
    	local cfg = S.Jump
    ...
    -- rows  = feather rows {n, s0,s1 (rise), sw0,sw1 (sweep), l0,l1, w0,w1, bend, curve, color, alpha, texture, texSpeed, z, jitter, emission}
    -- bones = fan of bones with membranes {n, s0,s1, sw0,sw1, l0,l1, w, color, alpha, mem (membrane colour), memAlpha, memTex, memSpeed, segs}
    local function wingSpec(style, pal)
    	...
    	if style == "Seraph" then
    		-- seraph: a giant fiery-gold fan upward + small cloak wings downward
    	...
    	elseif style == "Archangel" then
    		-- archangel: long white-gold feathers in three tiers, a calm majestic beat
    		spec.rows = {
    	...
    	elseif style == "Wyvern" then
    		-- wyvern: bones with a leathery membrane + a crest of spikes at the base
    	...
    	elseif style == "Nocturne" then
    		-- nocturne: long thin bones of the night + a black-blue membrane, a slow beat
    	...
    	elseif style == "Hummingbird" then
    		-- hummingbird: tiny boomerang wings, a blurry flutter
    		spec.rows = {
    	...
    	elseif style == "Stained Glass" then
    		-- stained glass: lead bones + rainbow membranes with seams
    	...
    	elseif style == "Inferno" then
    		-- inferno: two layers of living flame + a dense rain of heat
    	...
    	elseif style == "Abyss" then
    		-- abyss: giant bones in a smoke membrane, almost black, a rare beat
    	...
    	elseif style == "Pixie" then
    		-- fairy: transparent glass-like membranes, a fast flutter
    	...
    	elseif style == "Mantis" then
    		-- mantis: two scythe blades with a narrow edge
    	...
    	elseif style == "Tempest" then
    		-- storm: a double layer (thunder + echo) and lightning between the tips
    		spec.rows = {
    	...
    	elseif style == "Prism" then
    		-- prism: six rainbow blades in a fan
    		spec.rows = {}
    		for i = 1, 6 do
    			local h = (i - 1) / 6
    	...
    	elseif style == "Sakura" then
    		-- sakura: wide gentle petal-leaves + a petal rain
    	...
    	elseif style == "Skeletal" then
    		-- skeleton: bone ribs with a transparent film and a "spine" of dots
    	...
    	elseif style == "Solaris" then
    		-- sun: sun rays spread out in a fan, wide at the edge
    		spec.rows = {}
    		for i = 1, 8 do
    			local a = 82 - i * 17
    	...
    	elseif style == "Glacier" then
    		-- glacier: faceted ice blades + frosty tendrils
    		spec.rows = {
    	...
    	elseif style == "Wraith" then
    		-- ghost: smoky canvases with a burning edge + fumes
    		spec.rows = {
    	...
    	elseif style == "Aurora Veil" then
    		-- aurora: wide translucent curtains with a scrolling texture
    		...
    		-- moth: two large canvas wings with ring "eyespots" and smouldering dust
    		spec.rows = {
    ...
    -- ENERGY RING BEHIND THE WINGS (v2, upgraded): a universal FX kit + 6 styles.
    -- References: the Evangelion halo, magic circles/runes,
    ...
    -- FX kit (available in every style): comets (a runner with a tail), pulse waves
    ...
    local function makePropRing(torso, cfg, pal, scale, nRings)
    	-- Propulsion ring v3: SMOOTH glowing circles (Beam + CurveSize) +
    	-- a small glowing core (Parts). Few objects, a clean circle.
    	-- The SIZE depends ONLY on the ring slider (PropSize), NOT on the wing size.
    	local style  = cfg.PropStyle or "Halo"
    	local R      = 1.85 * (cfg.PropSize or 1)   -- base ring radius (not * scale!)
    	local spd    = (cfg.PropSpeed or 1) * 2.4
    	local obj    = { loops = {}, parts = {}, mov = {}, rim = {} }
    	local clock  = 0

    	-- colours
    	local hot = cseq(WHITE, pal.bright, pal.base)   -- "hot" glow of the circle
    	local soft = pal.seqFade                        -- soft echo
    	local coreGlow = pal.bright                     -- colour of the glowing core

    	-- ring plane: EDGE-ON behind the back. +Z of the torso = the back. The circle is built
    	-- by makeLoop in the local X-Z plane rotated +90 deg around X so that
    	-- its normal points backward (+Z) — a clean full circle is seen from behind.
    	...
    	-- helper: anchor part (follows the torso via obj.update)
    	...
    	-- helper: SMOOTH glowing ring (few Beams, CurveSize -> a clean circle)
    	...
    	-- helper: orbital "spark" (a bright dot on the ring that shows rotation)
    	local sparks = {}
    	...
    	-- small core in the middle of the circle (a glowing sphere + a thin inner ring)
    	...
    	if style == "Arc Reactor" then
    		-- main bright ring + inner outline ring + core + satellites
    		ring(R, 0.16, hot, {})
    	...
    	-- second ring (double): smaller and deeper
    	if (nRings or 1) >= 2 then
    		ring(R * 0.8, 0.08, soft, { alpha = 0.2 })
    		orbit(R * 0.8, 2, WHITE, 0.9, -1)
    	end

    	-- soft halo glow behind the circle (makes it a circle of light rather than a ribbon)
    	...
    	-- bind the initial position (so it does not flash at the origin before the tick)
    	...
    	function obj.update(dt, speedMul)
    		...
    		clock = clock + dt
    		-- the core/satellites follow the torso
    		for _, s in ipairs(obj.parts) do s.p.CFrame = W * s.cf end
    		-- sparks rotate around the circle (local ring plane)
    ...
    function Wings.build()
    	...
    	-- lightning between the tips (Tempest)
    	local bolts = {}
    	if spec.bolts then
    		for i = 1, #tips - 1 do
    	...
    	-- dust/sparks from the tips of the outer feathers
    	...
    	do -- v5.2 DETAILS: a light scatter of sparks by the wings
    	...
    	if cfg.Glow then
    		...
    		-- soft halo behind the back: one large sprite pinned to the torso (LockedToPart)
    	...
    	-- ENERGY RING BEHIND THE WINGS (halo / magic circle / arc reactor — PropRing)
    	local propRings = {}
    	if cfg.Prop then
    	...
    	Tick.use("Wings", function(dt, t)
    		...
    		-- dynamic pose: open and beating faster in the air, lowered when standing, folded when sitting
    ...
    function Halo.build()
    	...
    	Halo:keep(pivot)
    	do -- v5.2 DETAILS: living flicker around the halo
    	...
    	if style == "Corona" then
    		-- crown: a bright rim + a breathing halo + tongues of flame around
    	...
    	elseif style == "Broken Ring" then
    		-- broken ring: arcs of different radii drift and spin out of sync
    		local n = q(6, 4)
    		for i = 1, n do
    	...
    	elseif style == "Atom" then
    		-- atom: a core + three orbits + comet electrons with trails
    	...
    	do -- v5.3 CONSTRUCTOR: the atom core assembles from sphere parts
    	...
    	elseif style == "Spinner" then
    		-- turbine: three wide blades against a machined rim
    		local blades = q(3, 3)
    		for i = 1, blades do
    	...
    	elseif style == "Chandelier" then
    		-- chandelier: two rims and swinging crystal pendants with sparkles
    	...
    	elseif style == "Rune Wheel" then
    		-- rune wheel: a rim + a hexagonal weave + rune symbols around
    	...
    	do -- v5.3 CONSTRUCTOR: the wheel flies together from parts and spins
    	...
    	elseif style == "Saturn" then
    		-- saturn: a planet core, a fat tilted rim, two thin ones + an orbiting moon
    	...
    	elseif style == "Arc Reactor" then
    		-- reactor: concentric rings + spokes + a triangular core + a pulse
    	...
    	elseif style == "Crescent Moon" then
    		-- crescent: a bright sprite ring + an offset dark masking disc + stars
    	...
    	elseif style == "Cage" then
    		-- cage: vertical bars + top/bottom rims + a floating key glint
    	...
    	elseif style == "Petal Crown" then
    		-- wreath: petal sprites breathe (open/close) + petals fall
    		local petals = q(8, 6)
    		local made = {}
    		for i = 1, petals do
    	...
    	elseif style == "Vortex" then
    		-- funnel: spirals wind toward the core, dust is pulled inward
    		for i = 1, q(5, 3) do
    	...
    	elseif style == "Storm Eye" then
    		-- eye of the storm: a smoke funnel + a ring of crackling discharges
    	...
    	elseif style == "Gyroscope" then
    		-- gyroscope: three gimbal rings on different axes + a pendulum core
    	...
    	elseif style == "Stacked" then
    		-- stack: rings staggered, alternating rotation, soft breathing
    		for i = 1, q(4, 3) do
    	...
    	elseif style == "Glitch Ring" then
    		-- glitch: RGB splitting of segments, jerky jumps, a scan bar
    		local segs = q(8, 5)
    		for i = 1, segs do
    	...
    	elseif style == "Solar Disc" then
    		-- solar disc: a sprite disc + a corona + prominence glints + a thin rim
    		...
    		-- star crown: five spinning stars + a dotted line + a second star inside
    		local n = q(5, 4)
    		for i = 1, n do
    	...
    	Tick.use("Halo", function(dt, t)
    		-- "mood": at low HP the halo dims, tilts and slows down
    ...
    function Hat.build()
    	...
    	do -- v5.2 DETAILS: flickering motes around the hat
    	...
    	-- shell: a truncated cone. rTop/rBot/height in head units, y = bottom
    	...
    	local top = 0.6 -- top of the head

    	if style == "Top Hat" then
    		-- cylinder: a tall tube, brim, satin band and a bow shine
    	...
    	elseif style == "Wizard" then
    		-- wizard: a pointed bent hat, stars in a spiral, a starfall from the brim
    	...
    	elseif style == "Bucket" then
    		-- bucket: a body, a rim, a wire handle and water drops
    	...
    	elseif style == "Sombrero" then
    		-- sombrero: huge wavy brim, zigzag embroidery, tassels along the edge
    	...
    	elseif style == "Crown" then
    		-- crown: a rim with gems, points, ray glints on the peaks
    	...
    	elseif style == "Beanie" then
    		-- beanie: a knitted body with stripes, a swinging pompom
    	...
    	elseif style == "Antenna" then
    		-- antenna: a dome cap, two arc antennae and a BLINKING signal light
    	...
    	elseif style == "Chef" then
    		-- chef: a puffy hat with a porcelain sheen and steam from under the band
    	...
    	elseif style == "Pirate" then
    		-- pirate tricorn: a dome, two upturned brim wings, a skull with glowing eye sockets
    	...
    	elseif style == "Party Cone" then
    		-- party hat: a striped cone, a pompom and a confetti rain
    	...
    	elseif style == "Fez" then
    		-- fez: a cylinder with a swinging tassel and a thin band
    	...
    	elseif style == "Bamboo" then
    		-- bamboo hat: a wide cone with segments and leaves along the edge
    	...
    	elseif style == "Mushroom" then
    		-- mushroom: a stem, a spotted dome cap, spores and ground mist
    	...
    	elseif style == "Ice Crown" then
    		-- ice crown: faceted crystal peaks + frosty haze + snowflakes
    	...
    	elseif style == "Fire Crown" then
    		-- fire crown: a rim + living flames + a rain of coals
    	...
    	elseif style == "Nimbus" then
    		-- halo cloud: puffy clouds in a circle + a classic ring above them
    		for i = 1, q(6, 4) do
    	...
    	elseif style == "Storm Cloud" then
    		-- thundercloud: dark layers, drizzling rain, lightning flashes inside
    		for i = 1, 2 do
    		...
    		-- star hat: a sky dome + an orbit of stars + falling starlets
    	...
    	-- fringe (extra): hanging ribbons from the widest rim
    	if cfg.Fringe then
    		local n = q(7, 4)
    		local outer = 0
    ...
    function Glow.build()
    	...
    	local bloom = nil
    	if cfg.Bloom then
    		-- soft additive halo around the body: a large forcefield_glow sprite pinned to the torso
    ...
    function Aura.build()
    	...
    	if shape == "Orbit Rings" then
    		-- orbital rings: tilted hoops at different heights that breathe and spin out of sync
    	...
    	elseif shape == "Double Helix" then
    		-- double helix: two bead strands with trails + rungs between them
    	...
    	elseif shape == "Cage Bolts" then
    		-- discharge cage: vertical lightning rotates around, crackling
    	...
    	do -- v5.3 CONSTRUCTOR: the cage assembles from link parts
    	...
    	elseif shape == "Rising Glyphs" then
    		-- rising glyphs: symbols float up along the body and dissolve
    		for i = 1, n do
    	...
    	elseif shape == "Ember Pit" then
    		-- smouldering pit: a fire rim at the feet + rising coals + heat haze
    	...
    	elseif shape == "Snowfall" then
    		-- snowfall: snowflakes fall around with a wind drift
    	...
    	elseif shape == "Bubbles" then
    		-- bubbles: soap spheres rise and pop
    		for i = 1, n do
    	...
    	elseif shape == "Moths" then
    		-- moths: fireflies with a double wing-flash flutter around
    	...
    	elseif shape == "Sonar" then
    		-- sonar: pulse rings spread from the feet + echo dots
    		local rings = {}
    		for i = 1, 3 do
    	...
    	elseif shape == "Shattered Halo" then
    		-- shattered halo: arc shards spin around the head, drifting in height
    	...
    	do -- v5.3 CONSTRUCTOR: shards fly together into a broken ring
    	...
    	elseif shape == "Vines" then
    		-- vines: wavy light lashes grow upward, leaf glints
    	...
    	elseif shape == "Static" then
    		-- static: jerky interference nodes + an arc between random nodes
    		for i = 1, n do
    	...
    	elseif shape == "Power Up" then
    		-- power up: an energy pillar + sparks converge + charge rings at the feet
    	...
    	elseif shape == "Star Shower" then
    		-- starfall: stars streak arcs overhead and fall around
    		for i = 1, math.max(3, math.min(n, 6)) do
    	...
    	elseif shape == "Ground Fog" then
    		-- fog: rolling puffs near the ground, living breath
    		local fogs = {}
    		for i = 1, math.max(3, math.min(n, 6)) do
    		...
    		-- sakura drift: petals fly with the wind around the player
    		for i = 1, n do
    	...
    	Tick.use("Aura", function(dt, t)
    		...
    		-- v5.1 LIVELINESS: each node has its own tempo/phase (noise instead of a steady cycle)
    		for i, fn in ipairs(movers) do
    			fn(t + organicNoise(t * 0.8, i * 2.3) * 0.22 + i * 0.7, spd * (1 + 0.1 * organicNoise(t * 0.55, i * 5.1)))
    		end
    	end)
    	do -- v5.2 DETAILS: rising spark-moths on top of any aura
    ...
    -- ground stamp: a single flat sprite (VelocityPerpendicular) that lies and slowly fades
    ...
    local function footprint(cf, pal, size, style, yaw)
    	...
    	if style == "Ring Step" then
    		-- clean step: a neat ring + a micro glint
    	...
    	elseif style == "Paw Puff" then
    		-- paw: a pad + four toes + light fluff
    	...
    	elseif style == "Frost Print" then
    		-- frost print: a crystal snowflake + a frosty rim
    	...
    	elseif style == "Ripple Pad" then
    		-- water ripples: two spreading rings + a surface glint
    		for i = 1, 2 do
    	...
    	elseif style == "Star Stamp" then
    		-- star stamp: a star outline + glow
    	...
    	elseif style == "Ember Trail" then
    		-- ember trail: a scorched patch + ember sparks
    	...
    	elseif style == "Water Step" then
    		-- water step: rings + a tiny splash
    	...
    	elseif style == "Glyph Stamp" then
    		-- glyph: a swirl rune + a spreading seal ring
    	...
    	elseif style == "Smoke Puff" then
    		-- smoke puff: soft puffs with wind
    ...
    function Pet.build()
    	...
    	if style == "Lantern Fish" then
    		-- lantern fish: a shell body, a fan tail, a glowing lure
    	...
    	elseif style == "Drone" then
    		-- drone: a body, four blurred rotors, a scanner beam downward
    	...
    	elseif style == "Ghost" then
    		-- ghost: sheet folds, black eyes, smoke below, a secret flicker
    	...
    	elseif style == "Dragonling" then
    		-- dragonling: a chain of segments, webbed wings, fire breath
    		local n = q(8, 5)
    		local segs = {}
    	...
    	elseif style == "Cube" then
    		-- cube: three orthogonal frames spin at different speeds, a glitch core
    		for i = 1, 3 do
    	...
    	do -- v5.3 CONSTRUCTOR: the cube assembles from 8 corner parts
    	...
    	elseif style == "Comet" then
    		-- comet: an ice core, a fiery cone tail, a sparkly trail
    	...
    	elseif style == "Bat" then
    		-- bat: a teardrop body + two webbed wings beating in antiphase
    	...
    	elseif style == "Hummer" then
    		-- hummingbird: a tiny body, blurred boomerang wings, a long beak
    	...
    	elseif style == "Blob" then
    		-- slime: a jelly hemisphere breathes, drops run down, a glint eye
    	...
    	elseif style == "Sentinel" then
    		-- sentinel: an armoured louvre ring + an all-seeing eye with a pupil
    		for i = 1, 3 do
    	...
    	elseif style == "Nebula Ball" then
    		-- nebula: a vortex texture inside a translucent sphere + a star rain
    	...
    	elseif style == "Origami Bird" then
    		-- origami bird: flat wing faces with fold lines, a paper keel
    		local wings = {}
    		for side = -1, 1, 2 do
    	...
    	elseif style == "Lightning Core" then
    		-- storm core: a plasma ball + discharge arcs in all directions + a hum of sparks
    	...
    	elseif style == "Snowflake" then
    		-- snowflake: 6 rays with a frame of barbs, spinning in two planes
    		for arm = 0, 2 do
    	...
    	elseif style == "Wisp" then
    		-- wisp: a teardrop flame + a smouldering tail + rare flickers
    	...
    	elseif style == "Star Sprite" then
    		-- star sprite: a five-ray star + two satellites on tilted orbits
    		...
    		-- rain cloud: layered puffs, drizzling rain, a rare thunder flash
    	...
    	Tick.use("Pet", function(dt, t)
    		...
    		-- orient along the movement direction (in the root's local coordinates)
    	...
    	do -- v5.2 DETAILS: sparkles around the pet
    ...
    function Trails.build()
    	...
    	local function buildOn(parent, scaleW, tag)
    		...
    		if style == "Silk" then
    			-- silk: a smooth ribbon with satin edges
    		...
    		elseif style == "Blaze" then
    			-- flame: a fiery ribbon + a white core + embers from the tail
    		...
    		elseif style == "Frostbite" then
    			-- frostbite: an ice ribbon + frosty needle edges + snow dust
    		...
    		elseif style == "Galaxy" then
    			-- galaxy: a misty dark ribbon + a star core + scattered stars
    		...
    		elseif style == "Bubblegum" then
    			-- bubblegum: puffy alternating ribbons + soap bubbles
    		...
    		elseif style == "Umbra" then
    			-- twilight: a sooty dark ribbon + a glowing thread inside
    		...
    		elseif style == "Split Neon" then
    			-- neon split: two coloured rails + a white thread down the middle
    		...
    		elseif style == "Petals" then
    			-- petals: a wide gentle ribbon + a swarm of spinning petals
    		...
    		elseif style == "Datastream" then
    			-- data stream: dotted buses + falling glyphs + flicker
    			for i = -1, 1 do
    		...
    		elseif style == "Polar Light" then
    			-- aurora: a tall rippling curtain + a shimmering fringe
    		...
    		elseif style == "Comet Tail" then
    			-- comet tail: a cocoon core + a halo + a sheaf of sparks behind
    		...
    		elseif style == "Static" then
    			-- static: a torn interference ribbon + spark clicks
    		...
    		elseif style == "Phantom" then
    			-- phantom: a wide translucent mantle + melting haze
    		...
    		elseif style == "Spectrum" then
    			-- spectrum: a rainbow ribbon + prismatic glints
    		...
    		elseif style == "Twin Helix" then
    			-- double helix: two ribbons wave up and down in antiphase
    		...
    		elseif style == "Vapor" then
    			-- steam: a wide hazy ribbon + melting puffs
    		...
    		elseif style == "Stardust" then
    			-- stardust: a thin thread + a spinning swarm of starlets
    		...
    		elseif style == "Ember Wake" then
    			-- coal trail: a smouldering ribbon + falling coals + smoke
    		...
    		elseif style == "Stamp Path" then
    			-- stamps: a trail of symbol seals along the path (TextureMode.Static) + a backing ring
    			...
    			-- water: current ribbons + splash droplets + streaking glints
    	...
    	Tick.use("Trails", function(dt, t)
    		...
    		-- speed reaction: off when standing, longer when running
    ...
    local function killFX(pos, victim)
    	...
    	if style == "Soul Column" then
    		-- pillar of souls: a light column + rising wisp souls + ring waves
    	...
    	elseif style == "Supernova" then
    		-- supernova: a flipbook explosion + a triple wave + spoke rays along the horizon
    	...
    	elseif style == "Glass Shatter" then
    		-- showcase shattered: a web of cracks + shard needles + rainbow glints
    	...
    	elseif style == "Inferno Pit" then
    		-- hell pit: a fire rim + lava fountains + fumes + wings of heat
    		local fire = cseq(WHITE, pal.bright, pal.base, tint(pal.deep, -0.5))
    	...
    	elseif style == "Singularity" then
    		-- singularity: everything is pulled INWARD, collapse and a white burst
    		for i = 1, 2 do
    	...
    	elseif style == "Heart Pop" then
    		-- heart burst: a pulsing outline + a swarm of mini hearts + pink waves
    	...
    	elseif style == "Deep Freeze" then
    		-- permafrost: a crystal snowflake + ice needles + freezing mist
    	...
    	elseif style == "Delete" then
    		-- "removed from server": RGB slices + a glyph rain + a vertical scan
    	...
    	elseif style == "Rocket Salvo" then
    		-- salvo: three rockets fly up and bloom into fireworks
    		for i = 1, 3 do
    	...
    	elseif style == "Grave Bloom" then
    		-- the grave blooms: a cross of light + a soul ascends + the ground falls away
    	...
    	elseif style == "Sky Strike" then
    		-- orbital strike: a beam from the sky + a thunder flash + a ground split
    	...
    	elseif style == "Spirit Cage" then
    		-- spirit cage: converging rings + pillar bars + souls inside
    		for i = 1, 4 do
    	...
    	elseif style == "Ink Execution" then
    		-- ink execution: a giant blot + strands rising + dark haze
    	...
    	elseif style == "Clockwork" then
    		-- clockwork: a dial, hands, toothed gears, steam
    	...
    	elseif style == "Prism Cascade" then
    		-- prismatic cascade: a white split -> a spectrum fan -> a rainbow rain
    	...
    	elseif style == "Shockwave Dome" then
    		-- dome shockwave: a double front + a dust ring + ground tremor
    	...
    	elseif style == "Detonation" then
    		-- detonation: a crater + a layered explosion + a fire column + a mushroom cloud
    	...
    	elseif style == "Star Burst" then
    		-- star burst: a giant star outline + a swarm of stars + rays
    	...
    	elseif style == "Smoke Screen" then
    		-- smoke screen: dense puffs in a circle + smouldering coals inside
    		local n = q(8, 5)
    		for i = 1, n do
    			local a = i / n * TAU
    		...
    		-- wave: a rising wall of water in a circle + foam + spray + ring waves
    ...
    local AuraTrailer = defineModule("AuraTrailer", {
    	...
    	Lifetime = 0.55,   -- smoothness/tail length
    	Length   = 4,      -- trail length in studs
    	Width    = 1.1,    -- ribbon width
    	Sparks   = true,
    })
    AuraTrailer.ribbons = {}
    ...
    function AuraTrailer.build()
    	...
    	-- soft glow in the centre of the body
    ...
    -- Every asset style gets its OWN bright procedural fallback image so it is
    -- never empty or identical, even before the asset finishes loading.
    ...
    -- Built-in procedural styles (local textures, no network). Values —
    -- size/density/glow are raised so they are clearly visible.
    ...
    -- Applying a procedural (instant) style — bright and dense.
    local function applyBuiltin(style)
    	...
    	local function emitOn(part, scale)
    		...
    		-- v5.2 DETAILS: a flicker layer (tiny white flashes over the stream)
    		local e2 = emitter(a, {
    	...
    	-- v5.2: stream density "breathes" (organic noise, each with its own rhythm)
    ...
    -- Applying a template (asset) style from the cache.
    local function applyAsset(style, obj, color)
    	-- v5.1: if the cache/GetObjects returned a non-Instance — draw the procedural image
    ...
    function ParticleAura.build()
    	...
    	if ParticleAura.ids[style] then
    		...
    		-- the asset is not ready: this style gets its own bright procedural image
    ...
    local MotionEcho = defineModule("MotionEcho", {
    	...
    	Lifetime = 0.75,
    	Interval = 0.08,
    	Transparency = 0.45,   -- the lower, the denser/brighter the copies
    })
    MotionEcho.live = {}
    local lastPos, since, fadeAcc, totalParts = nil, 0, 0, 0
    local cloneBusy = false
    local HARD_CAP = 260   -- max total live ghost parts
    local SOFT_CAP = 180   -- above this no new clone starts until reset
    ...
    -- cache the list of source parts between ticks (for 0.25 s) so we do not
    -- run GetDescendants on EVERY frame
    ...
    -- take a pose snapshot once (fast), then clone with delays
    local function makeEcho()
    	...
    	-- snapshot: name/class/CFrame of every part — the pose is frozen before cloning starts
    	local snap = {}
    	...
    	for i, s in ipairs(snap) do
    		...
    		-- spread the heavy work across frames (no single-frame spike)
    	...
    	-- hard-reset the old ones if there are too many
    ...
    -- per-style settings: texture, size, physics, particle behaviour
    --   hue: each particle shifts the base colour hue slightly (a living shimmer)
    --   twk: flicker rate; grow: grow/shrink; fadeIn: soft appearance
    ScreenFX.spec = {
    ...
    -- per-style settings: texture, pixel size, speed, lifetime, gravity, stretch
    ScreenFX.spec = {
    ...
    ScreenFX.panel = { gui = nil, frame = nil } -- GUI state (keeps the chunk free of extra locals)
    ...
    function ScreenFX.spawnOne(spec, cfg, t)
    	...
    	img.Image = spec.tex
    	img.ZIndex = 5
    	-- particle colour: the base colour with a slight random hue shift (living shimmer)
    	local col = cfg.Color
    	if spec.hue then
    		local h, s, v = col:ToHSV()
    		if s < 0.02 then h = math.random() end -- almost white/black → random hue
    	...
    	if spec.radial then
    		-- speed lines: born at the centre and fly outward
    ...
    function ScreenFX.step(dt, t)
    	...
    	for i = 1, #list do
    		...
    		if a >= 1 then
    			...
    			-- transparency: soft fade-in -> life -> quick fade-out
    			local tr = a * a
    			if spec.fadeIn and a < 0.2 then tr = (0.2 - a) * 2.5 end
    			-- flicker: a living breathing sheen
    			if p.twk then
    ...
    Sky.Presets = {
    	...
    	["Golden Evening"] = {
    		-- recipe from Creator Docs (enhance outdoor environments): ClockTime 17, violet Ambient, Atmosphere 0.272 / Haze 1
    	...
    	["Candlelit Indoor"] = {
    		-- recipe from Creator Docs (enhance indoor environments): warm ambient, dense atmosphere, soft rays
    ...
    -- Skybox sets (face order: Bk, Dn, Ft, Lf, Rt, Up). "Plain Sky" is the built-in
    -- client skybox, "Indoor Studio" comes from the official Creator Docs tutorial,
    -- the rest are public catalogue sets gathered during research.
    ...
    -- Clouds (under Terrain): Cover 0-1, Density 0.1-0.3 per the docs recommendations
    Sky.CloudSpecs = {
    ...
    -- Filters on top of any sky: ColorCorrectionEffect (+ Bloom for some)
    Sky.Filters = {
    ...
    -- hide the game's Sky/Atmosphere/post-effects (the Default mode may have restored them)
    ...
    -- "Default" + the chosen skybox/clouds: game lighting stays, only the dome/clouds change
    local function applyDefaultExtras()
    	...
    	-- restore the game objects except the ones we replace (sky / clouds)
    ...
    -- skybox rotation (SkyboxOrientation is a newer Sky property; it quietly disables on old clients)
    Sky.spinAngle = 0
    ...
    -- wind gusts: workspace.GlobalWind = base + sines (per the docs); particles with Drag and
    -- WindAffectsDrag drift away, and Roblox terrain grass/leaves sway
    ...
    -- weather: 9 emitter points (centre + a 3x3 grid) follow the camera; an Attachment has no volume,
    -- so precipitation is spread across the grid. Extra handlers (thunder, aurora) live in Sky.weather.extra
    ...
    function Sky.setWeather(kind)
    	...
    	elseif w == "Aurora" then
    		-- an aurora of Beam curtains high above the camera: layers with different texture speeds
    		-- hide the repetition (a devforum tip), colour green -> turquoise -> violet
    		local curtains = {}
    		local colors = {
    		...
    		Sky.weather.extra = function(dt, t, p)
    			for _, c in ipairs(curtains) do
    				...
    				-- attachment X = along the ribbon, Y = upward (ribbon width = curtain height)
...
-- ========================================
-- Module: Shiftlock Crosshair (shiftlock_color)
-- ========================================
do
    ...
    -- Reworked version with improved pulses and new features:

    --   🌈 Rainbow/RGB mode (standalone HSV cycle)
    --   📐 Scale Pulse v2 — adjustable amplitude + waveforms + scale centre
    --   💗 Pulsate Transparency v2 — 4 waveforms (Sine/Breath/Triangle/Pulse) + min/max
    --   👻 Trail/Echo — residual ghost copies of the crosshair (trail effect)
    --   💾 Settings autosave (restored on reload)
    --   🎲 Random Preset / Random Color
    --   💾 Original object properties restored on disable

    -- Works only with the real odh_shared_plugins API:
    ...
    local state = {
        enabled = false,
        speed = SPEED_PRESETS[2].value,  -- Normal by default
    ...
    pulsate = false,
    pulseSpeed = 2,
    pulseWaveform = "sine",          -- waveform
    pulseDepth = 0.4,                -- max transparency (depth)

    -- 📐 Scale Pulse v2
    scalePulse = false,
    scaleAmplitude = 0.10,           -- how much it "breathes" (10% by default)

    -- 👻 Trail/Echo
    trailEnabled = false,
    trailCount = 2,                  -- number of ghost copies
    trailCopies = {},                -- created echo objects
    trailBuilding = false,           -- 🛡️ guard: prevents a second buildTrail call

    targetObjects = {},              -- found crosshair objects
    originalProps = {},              -- original object properties for restore
    propsByObj = {},                 -- [object] = record: O(1) lookup instead of a scan per frame
    rotation = 0,                    -- current gradient rotation angle
    offset = 0,                      -- current gradient offset
    pulsePhase = 0,                  -- pulse phase
    rainbowHue = 0,                  -- current rainbow hue

    renderConn = nil,                -- RenderStepped connection
    addedConn = nil,                 -- DescendantAdded connection
    removedConn = nil,               -- DescendantRemoving connection
    ...
    local WAVEFORM_FUNCS = {
        ...
        -- 💗 "Breath" — a slow inhale/exhale (sine cubed for softness)
        breath = function(phase)
            local s = math.sin(phase) * 0.5 + 0.5
            return s * s * (3 - 2 * s)  -- smoothstep for smoothness
        end,
    ...
    local function getActiveColors()
        if state.rainbowMode then
            return nil  -- built dynamically in RenderStepped
    ...
    local function isCrosshairObject(obj)
        ...
        -- 🛡️ Infinite-recursion guard: ignore trail clones
        -- (their names start with "crosshairtrail_"), otherwise DescendantAdded
        -- fires on the clone -> buildTrail() -> more clones -> crash
    ...
    local function saveOriginalProps(obj)
        ...
        record.grad = obj:FindFirstChild("CrosshairGradient")      -- cached: no per-frame lookup
    ...
    -- 🛡️ Hard limit: at most 8 copies per object (crash protection)
    ...
    for _, obj in ipairs(state.targetObjects) do
        for i = 1, maxPerObject do
            pcall(function()
                local clone = obj:Clone()
                -- unique name (the CrosshairTrail_ prefix is excluded in isCrosshairObject)
                clone.Name = "CrosshairTrail_" .. i
                -- clear the original gradient — the copy gets its own, created right away
                local g = clone:FindFirstChild("CrosshairGradient")
                if g then g:Destroy() end
                -- create its own gradient AT ONCE (not every frame!)
                local cg = Instance.new("UIGradient")
                cg.Name = "CrosshairGradient"
                -- copy colour/rotation from the source
                ...
                cg.Parent = clone
                -- higher transparency, the further the more transparent
                clone.ImageTransparency = 0.3 + (i / (maxPerObject + 1)) * 0.5
                -- ZIndex below the original
                clone.ZIndex = (obj.ZIndex or 1) - i
                -- place next to it (the position is set in RenderStepped)
    ...
    local function updateTrailPositions(scaleMul)
        local idx = 0
        for _, obj in ipairs(state.targetObjects) do
            -- find the record with the original properties
            local entry = nil
            ...
            if not entry then
                ...
                for i = 1, state.trailCount do
                    ...
                    if copy and copy.Parent then
                        pcall(function()
                            -- position offset — the copy is dragged along the scale motion
                            ...
                            -- position: the copy is offset slightly depending on its index
                            ...
                            -- 🛡️ sync colour/gradient with the original
                            -- WITHOUT creating new Instances — we only update the existing one
                            local srcGrad = obj:FindFirstChild("CrosshairGradient")
                            if srcGrad then
    ...
    -- Remove the gradients and restore the original properties
    for _, obj in ipairs(state.targetObjects) do
        pcall(function()
            local grad = obj:FindFirstChild("CrosshairGradient")
            if grad then
    ...
    state.targetObjects = {}
    state.originalProps = {}
    state.propsByObj = {}          -- keep the lookup map in sync with the array
    ...
    state.renderConn = ODHX.Connect(RunService.RenderStepped, function(dt)
        ...
        -- Compute the pulse value (0..1) with the selected waveform
        local waveVal = 0
        ...
        -- Transparency pulse (improved)
        local transparency = 0
        if state.pulsate then
            -- base wave from 0 to pulseDepth
            transparency = waveVal * state.pulseDepth
        end

        -- Scale Pulse (improved): amplitude
        local scaleMul = 1
        if state.scalePulse then
            scaleMul = 1 + waveVal * state.scaleAmplitude
        end

        -- One pcall for the whole pass (was one closure per object per frame) and
        -- no FindFirstChild in the loop: the gradient is cached per object.
        pcall(updateCrosshairObjects, transparency, scaleMul)

        -- 👆 Refresh the trail copies
        if state.trailEnabled and #state.trailCopies > 0 then
            updateTrailPositions(waveVal)
        end
    end)

    -- Auto-refresh of crosshair objects
    ...
    if playerGui then
        ...
        state.removedConn = ODHX.Connect(playerGui.DescendantRemoving, function(descendant)
            -- 🛡️ If a trail clone is removed, do not rebuild the trail (otherwise a loop)
...
-- Cleanup of the previous instance
...
-- =========================================================================
-- UI: tab + section through the odh_shared_plugins API
-- =========================================================================
...
runtime.handlers["Upgrade Mode"] = function(v)
    ...
    Reconfigure(true) -- cache reset, Sim/H/V values change
...
local function LoadSettings()
    ...
    if not data and err~="missing" then
        -- the main file failed: try the backup copy before falling back to defaults
...
runtime.connections[#runtime.connections+1]=Player.CharacterAdded:Connect(function(character)
    ...
    runtime.humanoid=nil          -- rebuild the cache for the new character
    StopCurrent("Respawned — select an emote and press Play")
end)
-- Humanoid comes from the runtime cache; FindFirstChildOfClass runs only when it is gone:
-- previously the instance search ran every frame while an emote played.
