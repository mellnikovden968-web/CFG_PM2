--[[════════════════════════════════════════════════════════════════════════════
    PM2 • ALL-IN-ONE  1.0
    Единая сборка плагинов для меню Overdrive H (API из my_plugin.lua)

    ЧТО ЭТО
      Все плагины пересобраны в ОДИН файл и рисуются в ОДНОЙ вкладке
      "PM2 All-in-One" с иконкой /mellnikovden968-web/CFG_PM2/refs/heads/main/icon
      (https://raw.githubusercontent.com/mellnikovden968-web/CFG_PM2/refs/heads/main/icon.png)

    КАК ЭТО УСТРОЕНО
      • Каждый плагин остался собой: его код лежит в отдельной функции
        Plugin_<TAG>() и выполняется в своей области видимости.
      • Обращение к хосту (odh_shared_plugins) заменено на ODH_HUB.For("<TAG>"):
        прокси отдаёт плагину общую вкладку вместо создания новой, поэтому
        вместо 11 вкладок получается одна.
      • Загрузка каждого плагина обёрнута в xpcall: если один плагин упадёт,
        остальные продолжат грузиться, а статус появится в секции "🧭 PM2 Hub".
      • У каждого плагина есть секция-заголовок "📦 <название>" со статусом,
        описанием и кнопкой «Выгрузить».

    НАСТРОЙКИ (файлы в рабочей папке экзекутора) у каждого плагина свои:
      Aimlock              -> ODH_Aimlock_settings.json
      Anims                -> ODH_FEAnimations_settings.json
      BJP                  -> ODH_BJP_settings.json
      BindableButtonsColor -> ODH_BindableButtonsColor_settings.json
      ButtonTransparency   -> ODH_ButtonTransparency_settings.json
      CFG (Omega)          -> Omega_CFG_settings.json
      Emotes               -> ODH_Emotes_settings.json (+ ODH_Emotes_catalog.json)
      PM_VALEX             -> ODH_PM_VALEX_settings.json (+ pm_valex.json)
      Pm-Wallhop           -> ODH_Pm-Wallhop_settings.json
      PrismFlux            -> ODH_PrismFlux_settings.json
      shiftlock_color      -> ODH_shiftlock_color_settings.json

    ПОВТОРНЫЙ ЗАПУСК ФАЙЛА
      Прошлые экземпляры плагинов выгружаются автоматически: реестр
      _G.ODH_2026_PluginRuntimes для адаптерных плагинов и рантаймы
      Anims / Emotes / CFG в _G. Вкладка создаётся заново, дубли не копятся.

    СЛУЖЕБНОЕ
      _G.ODH_PM2_HUB — доступ к хабу из консоли:
        ODH_PM2_HUB.StatusText(), ODH_PM2_HUB.Unload("PrismFlux"),
        ODH_PM2_HUB.UnloadAll(), ODH_PM2_HUB.results
════════════════════════════════════════════════════════════════════════════]]--

-- ═══════════════════════════ НАСТРОЙКИ СБОРКИ ═══════════════════════════════
local HUB_VERSION     = "1.0"
local HUB_TAB_NAME    = "PM2 All-in-One"          -- название общей вкладки
local HUB_ICON        = "/mellnikovden968-web/CFG_PM2/refs/heads/main/icon"    -- иконка: без https://raw.githubusercontent.com и без .png
local HUB_SECTION     = "🧭 PM2 Hub"       -- секция со статусами и общими кнопками
local HEADER_PREFIX   = "📦"               -- префикс секций-заголовков каждого плагина
local ADD_HEADERS     = true          -- секция-заголовок перед каждым плагином
local TAG_SUBTITLES   = true          -- помечать подзаголовок секции тегом плагина
local NOTIFY_ERRORS   = true          -- уведомления об ошибках загрузки

-- Порядок загрузки. Можно менять порядок или вычёркивать плагины — остальные
-- не пострадают. Плагины, перерисовывающие GUI меню, намеренно стоят последними.
local LOAD_ORDER = {
    "Aimlock",
    "Anims",
    "BJP",
    ...
    "PrismFlux",
    "shiftlock_color",
    "BindableButtonsColor",
    "ButtonTransparency",
}

-- Описания плагинов (текст для секций-заголовков).
local PLUGIN_INFO = {
    Aimlock = { tag = "Aimlock", title = "MM2 Aimlock", desc = "Авто-лок на убийцу в MM2: выбор игрока, wallcheck, сглаживание, FOV-круг, бинд-кнопки.", credit = "@anya_bts (V4, доработано SuperNinja)" },
    Anims = { tag = "Anims", title = "FE Animations", desc = "FE-анимации (пресеты aux0on, только R15): idle/walk/run/jump/climb/fall, полный сброс треков, автосохранение.", credit = "пресеты aux0on" },
    BJP = { tag = "BJP", title = "Bomb Jump+", desc = "Bomb Jump+ и Gold Bomb Jump+ для MM2: большие/бинд-кнопки, авто-подбор бомбы, кулдауны.", credit = "@lzzzx" },
    CFG = { tag = "CFG", title = "Omega Auto Revert", desc = "Авто-подстройка MM2_GPL под текущий пинг: адаптивный/классический режим, профили A-D, FPS Boost, мониторинг.", credit = "Omega Auto Revert" },
    Emotes = { tag = "Emotes", title = "Emotes (7yd7)", desc = "Библиотека эмотов и браузер карточек: воспроизведение, избранное, свои ID, R15.", credit = "7yd7" },
    PM_VALEX = { tag = "PM_VALEX", title = "PM VALEX • VOID RESET", desc = "Только MM2: reset игроков (шериф/убийца/все/выбранные/ближайшие), auto-режимы, списки, бинды, хоткеи, конфиг.", credit = "VALEX" },
    ["Pm-Wallhop"] = { tag = "Pm-Wallhop", title = "Pm-WallHop", desc = "WallHop и video flick: бинд-кнопка, размер/позиция, raycast-проверка стены.", credit = "@Phemtom (Improved)" },
    PrismFlux = { tag = "PrismFlux", title = "PrismFlux", desc = "Визуальные эффекты: Jump FX, крылья, гало, шляпа, небо, трейлы, Kill FX, Screen FX, пресеты цветов, профили.", credit = "" },
    shiftlock_color = { tag = "shiftlock_color", title = "Shiftlock Crosshair", desc = "Кастомизация прицела shiftlock (моб.): цвет, радуга, пульс, вращение, градиент, glow-пресеты, следы-копии.", credit = "" },
    BindableButtonsColor = { tag = "BindableButtonsColor", title = "Bindable Buttons Color", desc = "Перекраска бинд-кнопок меню: цвет фона/текста/обводки, контраст ON/OFF, пипетка, восстановление.", credit = "" },
    ButtonTransparency = { tag = "ButtonTransparency", title = "Button Transparency", desc = "Прозрачность кнопок и элементов меню хаба: процент, пипетка, восстановление исходного вида.", credit = "" },
}

-- Как выгружать каждый плагин: adapter = реестр ODH 2026, global = рантайм в _G.
local RUNTIME_SPECS = {
    Aimlock              = { adapter = "Aimlock" },
    Anims                = { global = "ODH_FEAnimationsRuntime_v2", method = "Cleanup" },
    BJP                  = { adapter = "BJP" },
    BindableButtonsColor = { adapter = "BindableButtonsColor" },
    ButtonTransparency   = { adapter = "ButtonTransparency" },
    CFG                  = { global = "OmegaAutoRevertRuntime",     method = "Cleanup" },
    Emotes               = { global = "ODH_7yd7_EmotesRuntime_v1",  method = "Cleanup" },
    PM_VALEX             = { adapter = "PM_VALEX" },
    ["Pm-Wallhop"]       = { adapter = "Pm-Wallhop" },
    PrismFlux            = { adapter = "PrismFlux" },
    shiftlock_color      = { adapter = "shiftlock_color" },
}

-- ══════════════════════════════ ЯДРО ХАБА ═══════════════════════════════════
local ODH_HUB = (function()
    local Hub = {
        version      = HUB_VERSION,
        host         = nil,
        tab          = nil,
    ...
    if type(host) ~= "table" or type(host.CreateTab) ~= "function" then
        warn("[PM2] odh_shared_plugins недоступен — загрузи этот файл через актуальное меню плагинов Overdrive H.")
        return nil
    end
    Hub.host = host
    ...
    local function firstLine(text)
        ...
        return (line:gsub("^%s+", "")):sub(1, 170)
    end

    function Hub.Notify(text, seconds)
        if type(host.Notify) == "function" then
            pcall(host.Notify, tostring(text), seconds or 4)
        end
    end

    local function makeLabel(section, text)
        local ok, handle = pcall(function() return section:AddLabel(text, true) end)
        if ok and type(handle) == "table" and type(handle.SetValue) == "function" then
            return handle
        end
        if ok and handle ~= nil then return nil end
        pcall(function() section:AddLabel(text) end)
        return nil
    end

    local function setLabel(handle, text)
        if handle == nil then return end
        pcall(handle.SetValue, handle, tostring(text))
    end
    ...
    -- Одна общая вкладка на все плагины
    local tabOk, tab = pcall(host.CreateTab, HUB_TAB_NAME, HUB_ICON)
    if not tabOk or tab == nil then
        warn("[PM2] CreateTab не сработал: " .. tostring(tab))
        return nil
    end
    Hub.tab = tab

    local function addSection(name, subtitle)
        local ok, section = pcall(function() return tab:AddSection(name, subtitle or "") end)
        if not ok or section == nil then
            warn("[PM2] AddSection не сработал для " .. tostring(name) .. ": " .. tostring(section))
            return nil
        end
        return section
    end

    -- ── Прокси хоста для одного плагина: CreateTab отдаёт общую вкладку ─────
    function Hub.For(tag)
        local cached = Hub.proxies[tag]
        if cached then return cached end
        Hub.counters[tag] = Hub.counters[tag] or { n = 0 }
        local counter = Hub.counters[tag]

        local wrapper = {}
        function wrapper:AddSection(name, subtitle)
            counter.n = counter.n + 1
            local sub = subtitle
            if TAG_SUBTITLES then
            ...
            local ok, section = pcall(function() return tab:AddSection(name, sub or "") end)
            if not ok or section == nil then
                warn("[PM2][" .. tag .. "] AddSection не сработал: " .. tostring(name) .. " / " .. tostring(section))
                return stubSection()
            end
            return section
        end
        setmetatable(wrapper, { __index = function(_, key) return tab[key] end })

        local proxy = setmetatable({
            CreateTab = function(_name, _icon) return wrapper end,
        }, { __index = host })
        Hub.proxies[tag] = proxy
        return proxy
    end

    -- ── Выгрузка предыдущего экземпляра (важно при повторном запуске файла) ─
    function Hub.PreClean(tag)
        local spec = RUNTIME_SPECS[tag]
        if not spec or not spec.global then return end
        local runtime = _G[spec.global]
        if type(runtime) ~= "table" then return end
        local method = runtime[spec.method]
        if type(method) == "function" then
            local ok, err = pcall(method)
            if not ok then
                warn("[PM2][" .. tag .. "] прошлый экземпляр не выгрузился: " .. tostring(err))
            end
        end
        pcall(function() _G[spec.global] = nil end)
    end

    function Hub.Unload(tag)
        local spec = RUNTIME_SPECS[tag]
        if not spec then return false, "нет описания выгрузки" end
        if spec.adapter then
            local registry = rawget(_G, "ODH_2026_PluginRuntimes")
            local runtime = type(registry) == "table" and registry[spec.adapter] or nil
            if type(runtime) == "table" and type(runtime.Stop) == "function" then
                local ok, err = pcall(runtime.Stop)
                if ok then return true end
                return false, err
            end
            return false, "рантайм не найден"
        end
        local runtime = _G[spec.global]
        if type(runtime) == "table" and type(runtime[spec.method]) == "function" then
            local ok, err = pcall(runtime[spec.method])
            if ok then return true end
            return false, err
        end
        return false, "рантайм не найден"
    ...
    function Hub.UnloadAll()
        local done, failed = 0, 0
        for index = #Hub.order, 1, -1 do
            local tag = Hub.order[index]
            local ok = Hub.Unload(tag)
            if ok then done = done + 1 else failed = failed + 1 end
            local rec = Hub.results[tag]
            if rec then
                rec.status = "unloaded"
                setLabel(rec.hubLabel, "⏹ " .. rec.title .. " — выгружен")
                setLabel(rec.headerLabel, "⏹ выгружен")
            end
        end
        return done, failed
    end

    function Hub.StatusText()
        local lines = { "PM2 All-in-One " .. tostring(Hub.version) .. " — статус плагинов:" }
        for _, tag in ipairs(Hub.order) do
            local rec = Hub.results[tag]
            if rec then
                lines[#lines + 1] = string.format("  %-21s %-8s секций: %-3d время: %.2f c%s",
                    tag, tostring(rec.status), rec.sections or 0, rec.time or 0,
                    rec.error and ("  | " .. firstLine(rec.error)) or "")
            end
        end
    ...
    -- ── Секция хаба ─────────────────────────────────────────────────────────
    Hub.section = addSection(HUB_SECTION, "все плагины в одной вкладке")
    if Hub.section then
        pcall(function()
            Hub.section:AddParagraph("PM2 All-in-One " .. HUB_VERSION,
                "Один файл — одна вкладка: плагины сборки грузятся сюда в порядке из LOAD_ORDER. "
                .. "У каждого плагина свой файл настроек и своя секция-заголовок "
                .. HEADER_PREFIX .. " с описанием и кнопкой выгрузки.")
        end)
        pcall(function()
            local credits = {}
            for _, tag in ipairs(LOAD_ORDER) do
                local info = PLUGIN_INFO[tag]
                if info and type(info.credit) == "string" and info.credit ~= "" then
                    credits[#credits + 1] = info.title .. " — " .. info.credit
                end
            end
            if #credits > 0 then
                Hub.section:AddParagraph("Авторы плагинов", table.concat(credits, "  •  ")
                    .. "  •  Сведено в одну вкладку сборкой PM2 All-in-One.")
            end
        end)
        Hub.summaryLabel = makeLabel(Hub.section, "⏳ загрузка сборки…")
        for _, tag in ipairs(LOAD_ORDER) do
            local info = PLUGIN_INFO[tag]
            local title = (info and info.title) or tag
            local rec = Hub.results[tag] or {}
            rec.tag = tag
            rec.title = title
            rec.status = "queued"
            rec.sections = 0
            rec.time = 0
            rec.hubLabel = makeLabel(Hub.section, "⏳ " .. title .. " — ожидание…")
            Hub.results[tag] = rec
        end
        pcall(function()
            Hub.section:AddButton("🧾 Статус в консоль", function()
                print(Hub.StatusText())
                Hub.Notify("PM2: статус выведен в консоль", 2)
            end)
        end)
        pcall(function()
            Hub.section:AddButton("⏹ Выгрузить все плагины", function()
                local done, failed = Hub.UnloadAll()
                local text = string.format("PM2: выгружено %d, не выгружено %d", done, failed)
                print(text)
                Hub.Notify(text, 4)
            end)
        end)
        local notifyToggle
        pcall(function()
            notifyToggle = Hub.section:AddToggle("Уведомления об ошибках", function(state)
                Hub.notifyErrors = state == true
            end)
        end)
        -- приводим вид переключателя к начальному значению NOTIFY_ERRORS
        if NOTIFY_ERRORS and type(notifyToggle) == "function" then pcall(notifyToggle) end
        pcall(function()
            Hub.section:AddLabel("Иконка: raw.githubusercontent.com" .. HUB_ICON .. ".png")
        end)
    end

    -- ── Запуск одного плагина ───────────────────────────────────────────────
    function Hub.Run(info, body)
        local tag = info.tag
        Hub.order[#Hub.order + 1] = tag
        Hub.counters[tag] = Hub.counters[tag] or { n = 0 }
        local counter = Hub.counters[tag]
        counter.n = 0
        ...
        if ADD_HEADERS then
            local header = addSection(HEADER_PREFIX .. " " .. info.title, HUB_TAB_NAME)
            rec.header = header
            if header then
                local descText = info.desc or ""
                if type(info.credit) == "string" and info.credit ~= "" then
                    descText = descText .. "   •   автор: " .. info.credit
                end
                pcall(function() header:AddParagraph(info.title, descText) end)
                rec.headerLabel = makeLabel(header, "⏳ загрузка…")
                pcall(function()
                    header:AddButton("⏹ Выгрузить " .. info.title, function()
                        local ok, err = Hub.Unload(tag)
                        if ok then
                            rec.status = "unloaded"
                            setLabel(rec.headerLabel, "⏹ выгружен")
                            setLabel(rec.hubLabel, "⏹ " .. info.title .. " — выгружен")
                            Hub.Notify("PM2: " .. info.title .. " выгружен", 3)
                        else
                            setLabel(rec.headerLabel, "❌ " .. firstLine(err))
                            Hub.Notify("PM2: " .. info.title .. " не выгрузился: " .. tostring(err), 4)
                        end
                    end)
                end)
            end
        ...
        local started = now()
        local ok, err = xpcall(body, function(e) return debug.traceback(tostring(e)) end)
        rec.time = now() - started
        rec.sections = counter.n

        local text
        if ok then
            if counter.n > 0 then
                rec.status = "loaded"
                text = string.format("✅ %s — загружен (секций: %d, %.2f c)", info.title, counter.n, rec.time)
            else
                rec.status = "skipped"
                text = string.format("⚠️ %s — пропущен (недоступен в этой игре или не создал секций)", info.title)
            end
        else
            rec.status = "error"
            rec.error = err
            text = string.format("❌ %s — ошибка: %s", info.title, firstLine(err))
            warn("[PM2][" .. tag .. "] " .. tostring(err))
            if Hub.notifyErrors then Hub.Notify(text, 6) end
        end
        print("[PM2] " .. text)
        setLabel(rec.hubLabel, text)
        setLabel(rec.headerLabel, text)
        return rec
    end
    ...
    function Hub.Finish()
        ...
        for _, tag in ipairs(Hub.order) do
            local rec = Hub.results[tag]
            if rec then
                if rec.status == "loaded" then loaded = loaded + 1
                elseif rec.status == "skipped" then skipped = skipped + 1
                elseif rec.status == "error" then failed = failed + 1 end
            end
        end
        local text = string.format("PM2 All-in-One: загружено %d, пропущено %d, с ошибкой %d (всего %d)",
            loaded, skipped, failed, #Hub.order)
        print("[PM2] " .. text)
        print(Hub.StatusText())
        Hub.Notify(text, 5)
        setLabel(Hub.summaryLabel, text)
    end

    rawset(_G, "ODH_PM2_HUB", Hub)
    return Hub
end)()

if type(ODH_HUB) ~= "table" then
    warn("[PM2] хаб не инициализирован — загрузка плагинов отменена.")
    return
end


-- ═══════════════════════════════════════════════════════════════════════════
-- ПЛАГИН: MM2 Aimlock   [Aimlock]   исходник: Aimlock.lua.txt
-- ═══════════════════════════════════════════════════════════════════════════
local function Plugin_Aimlock()
...
local ODHX = (function()
    ...
    X.id, X.title, X.file = "Aimlock", "MM2 Aimlock", "ODH_Aimlock_settings.json"
    local host = ODH_HUB.For("Aimlock")
...
-- MM2 Aimlock V4 — Murderer Auto-Lock + Player Select + Wallcheck + Smoothing + FOV
-- Improved from V3 by SuperNinja
-- Developer: @anya_bts | V4 Edition
--  - Cached character parts (no FindFirstChild every frame)
--  - Cached camera reference with auto-refresh on workspace change
--  - Smoothed prediction via Lerp (configurable smoothing) to remove camera jitter
--  - Per-frame validity checks for SelectedPlayer (leaving/respawn/death)
--  - Robust wallcheck (ignores accessories/decals, multi-point)
--  - Optional FOV check (only lock to targets within on-screen radius)
--  - Safe UI creation via pcall, fixed ripple connection leak
--  - Full Maid cleanup on unload, xpcall-guarded render loop
--  - Debounced gradient rotation (single shared RenderStepped)
---------------------------------------------------------------------------
...
-- Cached camera (refreshed when the workspace's CurrentCamera changes)
...
function BindableButtons.AddBButton(id, text, onFunc, offFunc)
    ...
    -- Debounced gradient rotation (cheaper than +1 deg/frame)
    local rotAccum = 0
    buttonMaid:GiveTask(ODHX.Connect(RunService.RenderStepped, function(dt)
        rotAccum = rotAccum + (dt * 90) -- ~90 deg/sec
        if rotAccum >= 1 then
            Gradient.Rotation = (Gradient.Rotation + rotAccum) % 360
            rotAccum = 0
        end
    end))
...
local Smoothness = 0.25        -- 0 = instant, 1 = very slow (Lerp alpha)
local FOVEnabled = false
local FOVRadius = 250          -- on-screen pixels
local FOVCircle = nil           -- Drawing-like circle drawn via GUI
local LastAimPos = nil          -- for Lerp smoothing
local LastTarget = nil          -- track target switches to reset smoothing

-- Horizontal (sideways) prediction often looks like the camera "drifting"
-- sideways when the murderer strafes. Off by default so the lock stays glued
-- to the body; vertical prediction is kept for jumps/falls.
local HorizontalPrediction = false
-- Smoothing speed in "converge per second" (higher = snappier).
-- We convert the Smoothness slider into a per-second rate below.
local SmoothRate = 18          -- tuned so default feels glued, not laggy

-- Per-target part cache so we don't FindFirstChild every frame
local TargetCache = {
    player = nil,
    char = nil,
    root = nil,
...
local function InvalidateCache()
    ...
    -- Also reset the smoothing anchor so the camera doesn't "slide" from the
    -- previous target's position when the target/cache changes.
    LastAimPos = nil
    LastTarget = nil
end
...
-- Resolve the aim part, using the cache where possible.
local function resolveAimPart(player)
    if not player or not player.Character then return nil end

    -- Refresh cache if the player or character changed
...
-- Robust wallcheck: ignores accessories/decals; multi-point raycast with a
-- majority vote so a single obscured limb doesn't drop the target.
local function isVisible(targetPart, targetPlayer)
    ...
    -- Build an exclude list. We must also exclude the local character's
    -- accessories and the target's accessories (they're separate instances
    -- parented under the character, so excluding the character covers them).
    local filterList = {}
    ...
    -- If we couldn't check anything, fall back to "visible" so the lock
    -- doesn't silently break on unusual rigs.
    if checked == 0 then return true end
    return visiblePoints >= (checked > 2 and 2 or 1)
end

-- FOV check: only lock when the target's screen position is within the radius.
...
local function getEffectiveTarget()
    if SelectedPlayer then
        ...
        -- Selected player invalid — drop and fall through to murderer
        SelectedPlayer = nil
    end
    return TargetPlayer
end
...
---------------------------------------------------------------------------
-- FOV Circle (lightweight, uses a Frame + UICorner so no Drawing API needed)
---------------------------------------------------------------------------
...
---------------------------------------------------------------------------
-- Bind Button using BindableButtons system
---------------------------------------------------------------------------
...
-- ODH Menu
---------------------------------------------------------------------------
section:AddLabel("Developer: @anya_bts | V4 Edition (improved)")
section:AddParagraph("Info", "Mode: HARD LOCK | Auto-Murderer | Wallcheck | Player Select | Smoothing | FOV")
...
section:AddSlider("🌀 Smoothness", 0, 95, 25, function(value)
    -- Slider value = "smoothness percent": 0 = instant (snapped), 95 = very slow.
    -- We map it to a per-second converge rate so behaviour is FPS-independent.
    -- 0%  -> rate = 1000 (effectively instant)
    -- 25% -> rate ~ 18  (default, glued but not laggy)
    -- 95% -> rate ~ 1   (very floaty)
    local pct = value / 100
    if pct <= 0.001 then
        SmoothRate = 1000
    else
...
-- Watch tools so the murderer is re-detected as soon as the knife is drawn.
...
-- Keep the bind button sized correctly when the viewport changes.
...
ToggleBindableVisibility()
UpdateFOVCircle()
shared.Notify("MM2 Aimlock V4 Loaded! (improved)", 3)
...
-- ═══════════════════════════════════════════════════════════════════════════
-- ПЛАГИН: FE Animations   [Anims]   исходник: Anims.lua.txt
-- ═══════════════════════════════════════════════════════════════════════════
local function Plugin_Anims()
...
local shared = ODH_HUB.For("Anims")
...
-- ═══════════════════════════════════════════════════════════════════════════
-- ПЛАГИН: Bomb Jump+   [BJP]   исходник: BJP.lua.txt
-- ═══════════════════════════════════════════════════════════════════════════
local function Plugin_BJP()
-- ODH 2026 adapter. Embedded in every plugin; no downloads/dependencies.
local ODHX = (function()
    local X = { ready=false, silent=false, restoring=false, replay=true, records={}, byKey={}, data={version=1, controls={}}, external=false }
    X.id, X.title, X.file = "BJP", "Bomb Jump+", "ODH_BJP_settings.json"
    local host = ODH_HUB.For("BJP")
    assert(host and type(host.CreateTab)=="function", X.title .. ": load through the current Overdrive H plugin menu")
    local env = {}
    if type(getgenv)=="function" then local ok,g=pcall(getgenv); if ok and type(g)=="table" then env=g end end
    local rd = type(readfile)=="function" and readfile or env.readfile
...
-- ═══════════════════════════════════════════════════════════════════════════
-- ПЛАГИН: Omega Auto Revert   [CFG]   исходник: CFG.lua.txt
-- ═══════════════════════════════════════════════════════════════════════════
local function Plugin_CFG()
--> Overdrive H On Top.
-- CFG / Omega: UI migrated to the API documented in my_plugin.lua.
-- Icon path: no https://raw.githubusercontent.com prefix and no .png suffix.
-- MM2 integration below is retained from CFG; it is NOT documented by my_plugin.lua.

local shared = ODH_HUB.For("CFG")
if not shared or type(shared.CreateTab) ~= "function" then
...
local reuseUI = type(previousRuntime) == "table"
    ...
    and false -- [PM2 merge] общая вкладка: UI всегда перестраивается
...
-- Persist only user preferences, never transient ping/configuration caches.
-- Executor filesystem: shared between Roblox sessions on this device.
...
local canPersist = type(fileRead) == "function" and type(fileWrite) == "function"
    and serviceOK and HttpService ~= nil
...
local function ReadPreferences()
    ...
    local ok, contents = pcall(fileRead, SETTINGS_FILE)
    if not ok then
        storage.status = "Read error"
        storage.issue = "Could not read " .. SETTINGS_FILE .. ": " .. tostring(contents)
        -- A missing file may be normal on first launch. Do not overwrite on
        -- startup when existence is uncertain; a user toggle can still save.
...
-- AddToggle returns a closure that FLIPS the toggle, not a SetValue method.
-- Remember the callback state and only call the closure when a flip is needed.
...
-- Confirmed from the live API's feature_name labels (2026-09-19).
-- The new API has seven consecutive sliders at slots 4..10; no slot 11.
local ConfigFields = {
...
-- New sliders are objects with :SetValue(number). Function setters are also
-- supported, but the slot map above targets the confirmed NEW API layout.
local function CallPreset(preset, slot, value, key)
    ...
    local ok, err = pcall(function()
        if method then
            setter(control, value) -- equivalent to control:SetValue(value)
...
-- Read-only diagnostics. Never call unknown functions to discover their role.
    ...
    local ok, err = pcall(function()
        ...
        for _, slot in ipairs(slots) do
            ...
            if type(control) == "table" then
                ...
                if type(name) == "string" then
                    -- Actual API label, not a name assumed from the old slot map.
...
local function Apply(cfg)
    ...
    -- Validate all seven controls BEFORE writing any slider/flag.
    -- If feature_name is exposed, verify its meaning, not just its type.
    for _, field in ipairs(ConfigFields) do
...
-- Dispatch through new runtime handlers even when reusing an older UI.
-- Save immediately on each toggle; do not depend on a leave/shutdown event.
for name in pairs(defaults) do
...
-- =========================================================================
-- UI: only signatures documented in my_plugin.lua are used here.
-- Every callback dispatches to ui.runtime so reloads never use old telemetry.
-- =========================================================================
...
if not reuseUI then
    local function RegisterToggle(name)
        -- New toggles start off. If the API emits an initial callback, use it.
    ...
    local ok, err = pcall(function()
        ...
        ui.section:AddParagraph(
            "About",
            "Adapts MM2_GPL settings to current ping. Adaptive mode interpolates control points; Classic uses profiles A-D. Disabling Auto Revert stops updates; it does not restore MM2 settings."
        )
        ui.section:AddParagraph(
            "Compatibility",
            "The UI uses odh_shared_plugins. Auto Revert additionally requires odh_internal_shared.MM2_GPL from the original CFG."
        )
...
-- Synchronize saved/default states via the documented toggle closures.
-- Initialization callbacks must not apply partial configurations.
for _, name in ipairs(names) do
...
-- ═══════════════════════════════════════════════════════════════════════════
-- ПЛАГИН: Emotes (7yd7)   [Emotes]   исходник: Emotes.lua.txt
-- ═══════════════════════════════════════════════════════════════════════════
local function Plugin_Emotes()
...
local shared = ODH_HUB.For("Emotes")
if not shared or type(shared.CreateTab)~="function" then
...
-- ═══════════════════════════════════════════════════════════════════════════
-- ПЛАГИН: PM VALEX • VOID RESET   [PM_VALEX]   исходник: PM_VALEX.lua.txt
-- ═══════════════════════════════════════════════════════════════════════════
local function Plugin_PM_VALEX()
-- ODH 2026 adapter. Embedded in every plugin; no downloads/dependencies.
local ODHX = (function()
    ...
    local host = ODH_HUB.For("PM_VALEX")
    assert(host and type(host.CreateTab)=="function", X.title .. ": load through the current Overdrive H plugin menu")
    local env = {}
    if type(getgenv)=="function" then local ok,g=pcall(getgenv); if ok and type(g)=="table" then env=g end end
    local rd = type(readfile)=="function" and readfile or env.readfile
...
-- единый кадр для всех кнопок (вместо N RenderStepped-подключений)
...
function BindableButtons.AddBButton(id, text, clickFunc, isGold)
    ...
    -- начальный размер задаём сразу, чтобы до первого кадра кнопка не была
    -- растянутой (дальше её каждый кадр пересчитывает binderStep)
    local camera = Workspace.CurrentCamera
    local screen = (camera and camera.ViewportSize) or FALLBACK_VIEWPORT
    ...
    -- drag + ripple + press одним подключением
    buttonMaid:GiveTask(ImageButton.InputBegan:Connect(function(input)
        if input.UserInputType ~= MOUSE1 and input.UserInputType ~= TOUCH then return end
        rec.targetPress = 1
        dragState = {
            rec = rec,
            input = input,                            -- именно этот палец/кнопка мыши
            dragInput = nil,                          -- для мыши заполнится из InputChanged
    ...
    -- Для мыши событие движения приходит отдельным InputObject (MouseMovement),
    -- поэтому запоминаем его здесь — в глобальном обработчике сверяется именно он.
    -- Для тача это тот же объект, что и в InputBegan.
...
function StatusHUD.Init()
    ...
    RootMaid:GiveTask(UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType ~= MOUSE1 and input.UserInputType ~= TOUCH then return end
        if hudDrag and hudDrag.moved then saveConfig() end
        hudDrag = nil
    end))

    -- пульс: обновляем ~30 раз/с, а не каждый кадр (экономия)
...
-- ====== Кэш ролей (remote ищется один раз) ======
local roleRemote = nil
local roleRemoteTried = -1e9   -- не 0: иначе при os.clock() < 5 первый поиск блокируется троттлингом
...
local function getCachedRoleData()
    ...
    roleCache.timestamp = t   -- не долбим сервер при ошибке
...
local function isValidTarget(player)
    ...
    if hum and hum.Health <= 0 then return false end   -- мёртвых не сбрасываем
    return true
end
...
local function findMurderer()
    ...
    -- запасной путь, если remote недоступен: ищем нож в руках
    local players = Players:GetPlayers()
    for i = 1, #players do
        local player = players[i]
        if player ~= LocalPlayer and not state.whitelist[player.UserId] then
...
-- только части тела (раньше CanCollide включался и у аксессуаров)
...
local function cancelCurrentReset()
    ...
    -- отменяем и запланированный ретрай: иначе «Cancel» не отменял повтор
...
local function VoidReset(TargetPlayer, _retryCount)
    ...
    -- кулдаун по цели (не мешает ретраям)
    ...
    -- цель реально сбрасывается — только теперь ставим метку кулдауна
    ...
    -- страховка: даже если кадр «зависнет», состояние восстановится
...
-- ====== Авто-режимы (одна реализация на все) ======
local function startAutoModule(maidKey, finderFunc, intervalFn)
    ...
    maid:GiveTask(thread)   -- при выключении поток гасится сразу, а не после wait
    return maid
end
...
local function stopAutoModule(maidKey)
    if maids[maidKey] then
        maids[maidKey]:Destroy()
        maids[maidKey] = nil
        -- глушим и начатый этим режимом сброс вместе с его ретраями:
        -- иначе выключенный авто-режим продолжал добивать цель
        cancelCurrentReset()
    end
end

-- ====== Действия (единый источник для меню, биндов и хоткеев) ======
local ACTIONS = {}
...
autoSection:AddToggle("Aura", function(enabled)
    ...
    startAutoModule("resetAura", function()
        ...
        local maxDistSq = (config.auraStuds or 15) ^ 2   -- без sqrt
...
autoSection:AddToggle("Click", function(enabled)
    ...
    local function resolveTarget()
        -- сначала современный raycast, GetMouse — только как запасной вариант
        local char = LocalPlayer.Character
...
listSection:AddPlayerDropdown("🛡 Un-whitelist", function(p)   -- новое
...
-- ====== Пометить свои карточки + повторные чистки ======
markOwnCards()

RootMaid:GiveTask(task.spawn(function()
    for _, delay in ipairs({ 1.5, 2.5, 3.0, 5.0 }) do
        task.wait(delay)
        pcall(markOwnCards)      -- сначала помечаем своё, потом чистим чужое
        pcall(purgeForeign, true)
    end
end))
...
if configLoaded then
    Notify(BRAND, VERSION .. " loaded (config restored)", 3)
else
    Notify(BRAND, VERSION .. " loaded. Duplicates auto-cleaned.", 3)
end
...
RootMaid:GiveTask(function()
    -- сначала запрещаем запись конфига: иначе отложенное сохранение (debounce)
    -- могло бы сохранить уже очищенный вайтлист
...
-- ====== Точка выгрузки ======
-- В V5.2 задача очистки лежала в RootMaid, но DoCleaning() никто не вызывал,
-- то есть выгрузка не происходила никогда. Теперь функция доступна хосту и
-- повторному запуску скрипта (повторный load выгружает прошлый экземпляр
-- в самом начале, до постройки нового интерфейса).
...
-- ═══════════════════════════════════════════════════════════════════════════
-- ПЛАГИН: Pm-WallHop   [Pm-Wallhop]   исходник: Pm-Wallhop.lua.txt
-- ═══════════════════════════════════════════════════════════════════════════
local function Plugin_Pm_Wallhop()
-- ODH 2026 adapter. Embedded in every plugin; no downloads/dependencies.
local ODHX = (function()
    local X = { ready=false, silent=false, restoring=false, replay=true, records={}, byKey={}, data={version=1, controls={}}, external=false }
    X.id, X.title, X.file = "Pm-Wallhop", "Pm-WallHop", "ODH_Pm-Wallhop_settings.json"
    local host = ODH_HUB.For("Pm-Wallhop")
    assert(host and type(host.CreateTab)=="function", X.title .. ": load through the current Overdrive H plugin menu")
    local env = {}
    if type(getgenv)=="function" then local ok,g=pcall(getgenv); if ok and type(g)=="table" then env=g end end
    local rd = type(readfile)=="function" and readfile or env.readfile
...
-- ═══════════════════════════════════════════════════════════════════════════
-- ПЛАГИН: PrismFlux   [PrismFlux]   исходник: PrismFlux.lua.txt
-- ═══════════════════════════════════════════════════════════════════════════
local function Plugin_PrismFlux()
	local PF_UI -- [PM2 merge] PF_UI остаётся локальным, чтобы не писать в _G
-- ODH 2026 adapter. Embedded in every plugin; no downloads/dependencies.
local ODHX = (function()
    local X = { ready=false, silent=false, restoring=false, replay=false, records={}, byKey={}, data={version=1, controls={}}, external=false }
    X.id, X.title, X.file = "PrismFlux", "PrismFlux", "ODH_PrismFlux_settings.json"
    local host = ODH_HUB.For("PrismFlux")
    assert(host and type(host.CreateTab)=="function", X.title .. ": load through the current Overdrive H plugin menu")
    local env = {}
    if type(getgenv)=="function" then local ok,g=pcall(getgenv); if ok and type(g)=="table" then env=g end end
    local rd = type(readfile)=="function" and readfile or env.readfile
...
-- количество элементов с учётом качества (не ниже minimum)
local function q(n, minimum)
	return math.max(minimum or 1, math.floor(n * QUALITY + 0.5))
end

-- Библиотека текстур.
--  • rbxasset://  — встроены в клиент (грузятся мгновенно, работают в любой игре)
--  • rbxassetid:// — официальные ассеты Roblox из туториалов Creator Docs
--    (Mansion of Wonder, Volcano/Waterfall tutorials) — общедоступные
local TEX = {
	...
	Boom    = "rbxassetid://17040521870", -- flipbook 8×8
}
...
-- контейнер настроек: каждый модуль кладёт сюда свой блок через defineModule
local S = {}
...
-- k > 0 тянет цвет к белому, k < 0 — к чёрному
local function tint(c, k)
	if k >= 0 then return c:Lerp(WHITE, math.min(k, 1)) end
	return c:Lerp(BLACK, math.min(-k, 1))
end

-- ColorSequence / NumberSequence из списка значений (равномерные ключи)
-- v5.1 ZHIVOST: органический шум (сумма несоизмеримых синусоид).
-- Движение не зациклено: у каждого эффекта (seed) свой характер темпа.
...
-- v5.1: cseq не роняет стиль на готовых ColorSequence (разворачивает в цвет)
local function cseq(...)
	local list = { ... }
...
-- палитра из одного цвета (или тройки Palette, или оттенка радуги)
local function makePalette(cfg, hue)
	...
	p.seq      = cseq(bright, base, deep)      -- светлое → тёмное
	p.seqUp    = cseq(deep, base, bright)      -- тёмное → светлое
	p.seqHot   = cseq(WHITE, bright, base)     -- белое ядро
...
-- сдвиг оттенка для радуги «на лету» (белое/чёрное не трогаем)
...
-- точка в мировых координатах: аттачмент на Terrain (Terrain стоит в нуле)
...
local function applyProps(inst, props)
	...
	for k, v in pairs(props) do
		if k ~= "Parent" then
			-- v5.1: Color3 для Color (ParticleEmitter/Beam/Trail) заворачивается сам
...
-- Дуга/кольцо из N кубических Безье. Span = доля окружности (1 = замкнутое кольцо).
-- loop:set(radius, width, alpha|nil, spin)  loop:color(seq)  loop:alpha(a)  loop:destroy()
...
-- Усечённый конус из панелей: rTop = 0 → конус, rTop = rBot → цилиндр, height = 0 → диск.
-- shell:set(centerCF, rTop, rBot, height, bulge, spin)
...
-- Перо крыла: лента от корня к кончику; spread — подъём, sweep — отвод назад, bend — изгиб
...
-- Молния: K звеньев с дрожанием. bolt:set(p0, p1, jitter, seed) — координаты в системе parent
...
-- расширяющаяся дуга/кольцо (Span < 1 → незамкнутая дуга)
...
-- веер дуг: N коротких дуг разлетаются от центра, каждая наклоняется наружу
...
-- столб света: лента к камере, растёт вверх; Twist > 0 → две переплетённые ленты
...
-- осколки: короткие ленты разлетаются баллистически и вращаются
...
-- поднимающаяся спиральная лента
...
-- расширяющийся купол (полусфера из конического сегмента)
...
-- радиальный веер частиц (k эмиттеров по кругу, наружу или внутрь)
...
-- спрайт, смотрящий в камеру: вспышка / звезда / блик с «хлопком» размера
...
-- flipbook-взрыв 8×8 (OneShot: вся сетка кадров проигрывается за время жизни частицы).
-- Если клиент не умеет Flipbook — откат на статичную текстуру ядра взрыва.
...
-- слоистый взрыв: ядро → ореол → искры (Drag + гравитация, вытянуты по скорости через Squash) → дым
local function burst(cf, pal, o)
	...
	-- v5.2 ДЕТАЛИ: донная ударная волна + россыпь блёсток по земле
...
-- круги на воде (рецепт из туториала водопада): плоские кольца, растущие и гаснущие
...
local PFPrim = {} -- кит фигур (одна внешняя таблица, чтобы не раздувать локалы чанка)
do

-- набор точек фигуры в единичном круге; возвращает СПИСОК подпутей (ломаных)
local function shapePaths(kind, seed)
	...
	elseif kind == "flake" then
		-- снежинка: 6 лучей с двумя усиками на каждом
		for arm = 0, 5 do
	...
	elseif kind == "flower" then
		-- 6 лепестков-капель + сердцевина-кольцо
		for p = 0, 5 do
	...
	elseif kind == "grid" then
		-- решётка 3×3 из линий
		for i = -1, 1 do
			pt(i * 0.62, -0.93) pt(i * 0.62, 0.93) openPath()
			pt(-0.93, i * 0.62) pt(0.93, i * 0.62) openPath()
		end
	elseif kind == "gear" then
		-- зубчатое колесо: попеременно внешний/внутренний радиус, 8 зубьев
		for i = 0, 32 do
			local a = i / 32 * TAU
		...
		openPath()
	else -- "ring": замкнутая окружность
...
-- анимированная фигура из лучей: прорисовка, вращение, подъём, пульс, затухание
local function polyFX(cf, o)
	...
	local draw = o.Draw or 0.45      -- доля dur на прорисовку
	local spin = o.Spin or 0
	local rise = o.Rise or 0
	local a0 = o.A0 or 0
	local glow = (o.Glow ~= false)
	local wall = (o.Plane == "wall") -- вертикальная плоскость (перпендикулярно взгляду cf)
	...
	-- v5.3 KONSTRUKTOR: вершины фигуры собираются из слетающихся деталей
	local asms
	if o.Assembly then
		asms = {}
		local seen = {}
	...
	Timeline.add(dur, function(a, dt, t)
		...
		-- v5.3 KONSTRUKTOR: детали слетаются в вершины и живут на них
		if asms then
...
-- трещины на земле: рваные радиальные ветки + пыль + тёмное пятно
...
-- шипы-«иглы» из земли: конические лучи ВЫРАСТАЮТ наружу (не разлетаются)
...
-- комета: светящееся ядро летит по дуге из p0 в p1, шлейф, взрыв при падении
...
-- чернильная клякса: тёмное пятно + жгуты-щупальца + брызги по плоскости
...
-- «матричный» дождь: падающие штрихи-глифы в цилиндре
...
-- паутина молний: разряды между точками кольца + вспышка в центре
...
-- пульсирующее сердце-контур + всплывающие мини-сердца
...
-- RGB-глитч: горизонтальные срезы трёх цветов дрожат и гаснут
...
-- рунный круг: вращающийся сворл + штрихи + двойной гексагон
...
-- снежинка-кристалл: 6-лучевой каркас растёт и искрится
...
-- огненные крылья: два взмаха-силуэта из слоёв пламени
...
-- призма: белый vendors → веер цветных лучей + радужные кольца
...
-- часовой механизм: циферблат-сегменты, две стрелки, тики
local function clockFX(cf, o)
	...
	-- тики: короткие вспышки на 12 и 6
	for k = 1, 4 do
...
-- восходящие души-«виспы» с глазками-тенями
local function soulFX(cf, o)
	...
	-- v5.3 KONSTRUKTOR: постоянная сборка — детали слетаются в 3D-фигуру
	-- и живут на ней (микро-орбита + мерцание, без машинного повтора).
	-- kinds: ring / oval / sphere / cage / box / heart; возвращает (список, update)
...
-- один Heartbeat на все постоянные визуалы. Tick.spin/wobble — общий idle-spin,
-- Tick.cap — ограничение частоты анимации (секция Performance).
...
-- текущий оттенок радуги по цели (заполняется системой пресетов)
local RainbowHue = {}
...
-- Реестр модулей. Модуль = настройки S[key] + инстансы + перекрасчики + build/stop.
-- Постоянные модули пересобираются при респауне/смене качества автоматически.
-- v5.2 ДЕТАЛИ: второй план постоянных эффектов — мерцающие искры-моты вокруг
-- носителя с «дыханием» плотности. Живёт в PFPrim, чтобы не раздувать локалы.
do
	local detailSeq = 0
...
local function defineModule(key, defaults, opts)
	...
	-- радуга без пересборки: сдвиг оттенка всех Beam/Particle/Trail/Light с запоминанием исходных цветов
	...
	-- стандартная остановка: снять тик и уничтожить инстансы (модуль может переопределить)
	function m.stop()
		Tick.drop(key)
		m:clear()
	end
...
local function jumpStyle(style, cf, pal, size, dur, sparks)
	if style == "Ripple" then
		-- вода: три концентрических кольца + «корона» из капель + блестящая гладь
		for i = 1, 3 do
	...
	elseif style == "Twin Arc" then
		-- два полумесяца-слэша крест-накрест + искра в точке скрещивания
		for i = 0, 1 do
	...
	elseif style == "Spiral Lift" then
		-- двойная спираль уносит вверх + присасывающиеся к центру пылинки
	...
	elseif style == "Shard Burst" then
		-- кристаллы ВЫРАСТАЮТ из земли веером и тают + стеклянная пыль
	...
	elseif style == "Beacon" then
		-- сигнальный столб: ядро-колонна + взлетающие кольца + маячок сверху
	...
	elseif style == "Dome" then
		-- купол-вспышка + гексагональная печать у земли + пыль внутри
	...
	elseif style == "Meteor Ring" then
		-- метеоры падают по кругу ВНУТРЬ и взрываются в центре
		local n = q(6, 4)
		for i = 1, n do
	...
	elseif style == "Thunder Step" then
		-- разлом + паутина разрядов + озоновые искры
	...
	elseif style == "Bloom" then
		-- цветок: 6 лепестков прорисовываются, сердцевина вспыхивает, пыльца летит
	...
	elseif style == "Crescent" then
		-- лунный серп: два кольца со смещением дают месяц + вертикальный слэш
	...
	elseif style == "Gear" then
		-- шестерня: зубчатый контур крутится, ступица вспыхивает, пар из пазов
	...
	elseif style == "Glass Break" then
		-- разбитое стекло: трещины + осколочные иглы + радужные блики
	...
	elseif style == "Ink Drop" then
		-- клякса: пятно расползается жгутами, брызги, дым-морок
	...
	elseif style == "Star Sigil" then
		-- пентаграмма прорисовывается + встречная звезда + руны на вершинах
	...
	elseif style == "Pulse Grid" then
		-- кибер-решётка: линии прорисовываются от центра, узлы мерцают
	...
	elseif style == "Phoenix" then
		-- феникс: огненные крылья-взмахи + столб пламени + дождь углей
	...
	elseif style == "Nova" then
		-- компакт-нов: flipbook-взрыв + ударная волна + слоистый выброс
	...
	elseif style == "Splash" then
		-- всплеск: водяная корона + капли по баллистике + круги на воде
	...
	elseif style == "Stardust" then
		-- звёздная пыль: рой звёзд-спрайтов закручивается вихрем
		...
		-- дымовое кольцо-тор: пухлые клубы разлетаются и тают + тень-подложка
...
-- прыжок в воздухе (двойной прыжок в играх с ним): маленькое кольцо под ногами прямо в воздухе
function Jump.air()
	local cfg = S.Jump
...
-- rows  = ряды перьев {n, s0,s1 (подъём), sw0,sw1 (отвод), l0,l1, w0,w1, bend, curve, color, alpha, texture, texSpeed, z, jitter, emission}
-- bones = веер костей с перепонками {n, s0,s1, sw0,sw1, l0,l1, w, color, alpha, mem (цвет перепонки), memAlpha, memTex, memSpeed, segs}
local function wingSpec(style, pal)
	...
	if style == "Seraph" then
		-- серафим: гигантский огненно-золотой веер вверх + малые крылья-плащи вниз
	...
	elseif style == "Archangel" then
		-- архангел: длинные бело-золотые перья в три яруса, спокойный величавый взмах
		spec.rows = {
	...
	elseif style == "Wyvern" then
		-- виверна: кости с кожистой перепонкой + гребень шипов у основания
	...
	elseif style == "Nocturne" then
		-- ноктюрн: длинные тонкие кости ночи + чёрно-синяя перепонка, медленный мах
	...
	elseif style == "Hummingbird" then
		-- колибри: крошечные крылья-бумеранги, трепет с размытием
		spec.rows = {
	...
	elseif style == "Stained Glass" then
		-- витраж: свинцовые кости + радужные перепонки со швами
	...
	elseif style == "Inferno" then
		-- инферно: два слоя живого пламени + плотный жар-дождь
	...
	elseif style == "Abyss" then
		-- бездна: исполинские кости в дым-мембране, почти чёрные, редкий взмах
	...
	elseif style == "Pixie" then
		-- фея: прозрачные перепонки-стеклышки, быстрое трепетание
	...
	elseif style == "Mantis" then
		-- богомол: две косы-клинка с узкой кромкой
	...
	elseif style == "Tempest" then
		-- буря: двойной слой (гроза + эхо) и молнии между кончиками
		spec.rows = {
	...
	elseif style == "Prism" then
		-- призма: шесть радужных лопастей веером
		spec.rows = {}
		for i = 1, 6 do
			local h = (i - 1) / 6
	...
	elseif style == "Sakura" then
		-- сакура: широкие нежные листья-лепестки + лепестковый дождь
	...
	elseif style == "Skeletal" then
		-- скелет: костные рёбра с прозрачной плёнкой и «хребтом»-точками
	...
	elseif style == "Solaris" then
		-- солнце: лучи-солнца расходятся веером, широкие у края
		spec.rows = {}
		for i = 1, 8 do
			local a = 82 - i * 17
	...
	elseif style == "Glacier" then
		-- ледник: гранёные ледяные лопасти + морозные усики
		spec.rows = {
	...
	elseif style == "Wraith" then
		-- призрак: дымные полотна с горящей кромкой + чад
		spec.rows = {
	...
	elseif style == "Aurora Veil" then
		-- аврора: широкие полупрозрачные занавесы с бегущей текстурой
		...
		-- мотылёк: два крупных крыла-полотна с «глазками»-кольцами и тлеющая пыль
		spec.rows = {
...
-- ЭНЕРГОКОЛЬЦО ЗА КРЫЛЬЯМИ (v2, прокачанная): универсальный FX-кит + 6 стилей.
-- Референсы: Evangelion-нимб (energo-koltso za golovoy), magicheskie krugi/runy,
-- Arc Reactor (segmentirovannye dugi), solnechnaya korona s protuberancami,
-- chernaya dyra/vortex so spiralnymi rukavami, shockwave-pulsacii.
-- FX-кит (dostupen v kazhdom stile): komety (runner s hvostom), puls-volny
-- (rasshiryayuschiesya dubl-kolca), molnii mezhdu segmentami, orbitery-iskry,
-- protuberancy (ognennye dugi), spiralnye rukava, svechenie-dyhanie.
-- obj.update(dt, speedMul) animiruet, obj.rim — beams dlya perekpraski.
...
local function makePropRing(torso, cfg, pal, scale, nRings)
	-- Пропульсионное кольцо v3: ГЛАДКИЕ светящиеся круги (Beam + CurveSize) +
	-- небольшое светящееся «ядро» (Parts). Мало объектов, чистая окружность.
	-- РАЗМЕР зависит ТОЛЬКО от слайдера кольца (PropSize), НЕ от размера крыльев.
	local style  = cfg.PropStyle or "Halo"
	local R      = 1.85 * (cfg.PropSize or 1)   -- базовый радиус кольца (не * scale!)
	local spd    = (cfg.PropSpeed or 1) * 2.4
	local obj    = { loops = {}, parts = {}, mov = {}, rim = {} }
	local clock  = 0

	-- цвета
	local hot = cseq(WHITE, pal.bright, pal.base)   -- «горячее» свечение круга
	local soft = pal.seqFade                        -- мягкое эхо
	local coreGlow = pal.bright                     -- цвет светящегося ядра

	-- плоскость круга: РЕБРОМ за спиной. +Z торса = спина. Круг построен
	-- makeLoop в локальной X-Z плоскости, повёрнутой вокруг X на +90°, чтобы
	-- его нормаль смотрела назад (+Z) — сзади виден чистый полный круг.
	...
	-- помощник: якорная деталь (следует за торсом через obj.update)
	...
	-- помощник: ГЛАДКОЕ светящееся кольцо (мало Beam, CurveSize → чистый круг)
	...
	-- помощник: орбитальная «искра» (яркая точка на кольце, показывает вращение)
	local sparks = {}
	...
	-- малое «ядро» в центре круга (светящаяся сфера + тонкое внутреннее кольцо)
	...
	if style == "Arc Reactor" then
		-- основное яркое кольцо + внутреннее контур-кольцо + ядро + спутники
		ring(R, 0.16, hot, {})
	...
	-- второе кольцо (двойное): меньше и глубже
	if (nRings or 1) >= 2 then
		ring(R * 0.8, 0.08, soft, { alpha = 0.2 })
		orbit(R * 0.8, 2, WHITE, 0.9, -1)
	end

	-- мягкий ореол-свечение за кругом (делает его «кругом света», а не лентой)
	-- [PM2 merge] исправлена опечатка оригинала: у Vector3 поля X/Y/Z (заглавные)
	...
	-- привязка начальной позиции (чтобы не мелькнуло в нуле до тика)
	...
	function obj.update(dt, speedMul)
		...
		clock = clock + dt
		-- ядро/спутники следуют за торсом
		for _, s in ipairs(obj.parts) do s.p.CFrame = W * s.cf end
		-- вращение искр по кругу (локальная плоскость кольца)
...
function Wings.build()
	...
	-- молнии между кончиками (Tempest)
	local bolts = {}
	if spec.bolts then
		for i = 1, #tips - 1 do
	...
	-- пыль/искры с кончиков крайних перьев
	...
	do -- v5.2 ДЕТАЛИ: лёгкая россыпь искр у крыльев
	...
	if cfg.Glow then
		...
		-- мягкий ореол за спиной: один большой спрайт, «прибит» к торсу (LockedToPart)
	...
	-- ЭНЕРГОКОЛЬЦО ЗА КРЫЛЬЯМИ (нимб / магический круг / арк-реактор — PropRing)
	local propRings = {}
	if cfg.Prop then
	...
	Tick.use("Wings", function(dt, t)
		...
		-- динамическая поза: в воздухе раскрыты и машут быстрее, стоя — приспущены, сидя — сложены
...
function Halo.build()
	...
	Halo:keep(pivot)
	do -- v5.2 ДЕТАЛИ: живое мерцание вокруг нимба
	...
	local loops, parts = {}, {}   -- loops: {loop, r, w, y, tilt, roll, spinMul, bob, pulse, wobbleAxis, span}, parts: updaters(t, spin, wob)
	...
	if style == "Corona" then
		-- корона: яркий обод + дышащий ореол + языки пламени по кругу
	...
	elseif style == "Broken Ring" then
		-- разбитое кольцо: дуги разных радиусов дрейфуют и крутятся вразнобой
		local n = q(6, 4)
		for i = 1, n do
	...
	elseif style == "Atom" then
		-- атом: ядро + три орбиты + электроны-кометы со шлейфами
	...
	do -- v5.3 KONSTRUKTOR: ядро атома собирается из деталей-сфер
	...
	elseif style == "Spinner" then
		-- турбина: три широких лопасти против фрезерованного обода
		local blades = q(3, 3)
		for i = 1, blades do
	...
	elseif style == "Chandelier" then
		-- люстра: два обода и качающиеся хрустальные подвесы с блёстками
	...
	elseif style == "Rune Wheel" then
		-- рунное колесо: обод + шестигранная вязь + руны-символы по кругу
	...
	do -- v5.3 KONSTRUKTOR: колесо слетается из деталей и вращается
	...
	elseif style == "Saturn" then
		-- сатурн: планета-ядро, жирный наклонный обод, два тонких + луна на орбите
	...
	elseif style == "Arc Reactor" then
		-- реактор: концентр + спицы + треугольное ядро + пульс
	...
	elseif style == "Crescent Moon" then
		-- месяц: яркое кольцо-спрайт + тёмный диск-маска со смещением + звёзды
	...
	elseif style == "Cage" then
		-- клетка: вертикальные прутья + ободы сверху/снизу + парящий ключ-блик
	...
	elseif style == "Petal Crown" then
		-- венок: лепестки-спрайты дышат (раскрыв/закрытие) + лепестки опадают
		local petals = q(8, 6)
		local made = {}
		for i = 1, petals do
	...
	elseif style == "Vortex" then
		-- воронка: спирали закручиваются к ядру, пыль затягивает внутрь
		for i = 1, q(5, 3) do
	...
	elseif style == "Storm Eye" then
		-- око бури: дымовая воронка + кольцо разрядов с треском
	...
	elseif style == "Gyroscope" then
		-- гироскоп: три кольца-карда на разных осях + ядро-маятник
	...
	elseif style == "Stacked" then
		-- этажерка: кольца лесенкой, чередование вращения, мягкое дыхание
		for i = 1, q(4, 3) do
	...
	elseif style == "Glitch Ring" then
		-- глитч: RGB-расщепление сегментов, дёрганые скачки, скан-полоса
		local segs = q(8, 5)
		for i = 1, segs do
	...
	elseif style == "Solar Disc" then
		-- солнечный диск: диск-спрайт + корона + протуберанцы-блики + тонкий обод
		...
		-- корона звёзд: пять вращающихся звёзд + пунктир + вторая звезда внутри
		local n = q(5, 4)
		for i = 1, n do
	...
	Tick.use("Halo", function(dt, t)
		-- «настроение»: при низком HP нимб тускнеет, кренится и замедляется
...
function Hat.build()
	...
	do -- v5.2 ДЕТАЛИ: мерцающие пылинки вокруг шляпы
	...
	-- оболочка: усечённый конус. rTop/rBot/height в единицах головы, y — низ
	...
	local top = 0.6 -- верх головы

	if style == "Top Hat" then
		-- цилиндр: высокая труба, поля, атласная лента и блеск-бабочка
	...
	elseif style == "Wizard" then
		-- колдун: острый колпак с изгибом, звёзды по спирали, звёздопад с кромки
	...
	elseif style == "Bucket" then
		-- ведро: корпус, обод, проволочная дужка и капли воды
	...
	elseif style == "Sombrero" then
		-- сомбреро: огромные поля с волной, вышивка-зигзаг, кисточки по краю
	...
	elseif style == "Crown" then
		-- корона: обод с самоцветами, зубцы, лучи-блики на вершинах
	...
	elseif style == "Beanie" then
		-- шапка-бини: вязаное полотно с полосками, помпон-качели
	...
	elseif style == "Antenna" then
		-- антенна: купол-шапка, две дуги-усика и МОРГАЮщий сигнальный огонь
	...
	elseif style == "Chef" then
		-- повар: пышный колпак с фарфоровым отблеском и пар из-под гребня
	...
	elseif style == "Pirate" then
		-- пиратская треуголка: купол, два загнутых поля-крыла, череп со светящимися глазницами
	...
	elseif style == "Party Cone" then
		-- праздничный колпак: полосатый конус, помпон и дождь конфетти
	...
	elseif style == "Fez" then
		-- феска: цилиндр с кисточкой-маятником и тонкой каймой
	...
	elseif style == "Bamboo" then
		-- бамбуковая шляпа: широкий конус с «сегментами» и листья по краю
	...
	elseif style == "Mushroom" then
		-- гриб: ножка, крапчатая шляпка-купол, споры и туман у земли
	...
	elseif style == "Ice Crown" then
		-- ледяная корона: гранёные пики-кристаллы + морозная дымка + снежинки
	...
	elseif style == "Fire Crown" then
		-- огненная корона: обод + живые языки пламени + угольный дождь
	...
	elseif style == "Nimbus" then
		-- нимб-облако: пухлые облака по кругу + классическое кольцо над ними
		for i = 1, q(6, 4) do
	...
	elseif style == "Storm Cloud" then
		-- грозовая туча: тёмные слои, моросит дождь, вспышки молний внутри
		for i = 1, 2 do
		...
		-- звёздная шляпа: купол-небо + орбита звёзд + падающие звёздочки
	...
	-- бахрома (доп.): свисающие ленты с самого широкого обода
	if cfg.Fringe then
		local n = q(7, 4)
		local outer = 0
...
function Glow.build()
	...
	local bloom = nil
	if cfg.Bloom then
		-- мягкий аддитивный ореол вокруг тела: крупный спрайт forcefield_glow, прибит к торсу
...
function Aura.build()
	...
	if shape == "Orbit Rings" then
		-- орбитальные кольца: наклонные обручи на разных высотах, дышат и крутятся вразнобой
	...
	elseif shape == "Double Helix" then
		-- двойная спираль: две пряди-бусины со шлейфами + перекладины между ними
	...
	elseif shape == "Cage Bolts" then
		-- клетка разрядов: вертикальные молнии вращаются вокруг, трещат
	...
	do -- v5.3 KONSTRUKTOR: клетка собирается из деталей-звеньев
	...
	elseif shape == "Rising Glyphs" then
		-- восходящие глифы: символы всплывают вдоль тела и растворяются
		for i = 1, n do
	...
	elseif shape == "Ember Pit" then
		-- тлеющая яма: огненный обод у ног + восходящие угли + жар-дымка
	...
	elseif shape == "Snowfall" then
		-- снегопад: снежинки падают вокруг, ветер-дрейф
	...
	elseif shape == "Bubbles" then
		-- пузыри: мыльные сферы всплывают и лопаются
		for i = 1, n do
	...
	elseif shape == "Moths" then
		-- мотыльки: огоньки с двойным крылом-всплеском порхают вокруг
	...
	elseif shape == "Sonar" then
		-- сонар: кольца-импульсы расходятся от ног + эхо-точки
		local rings = {}
		for i = 1, 3 do
	...
	elseif shape == "Shattered Halo" then
		-- разбитый нимб: дуги-осколки вращаются вокруг головы, дрейфуя по высоте
	...
	do -- v5.3 KONSTRUKTOR: осколки слетаются в разломанное кольцо
	...
	elseif shape == "Vines" then
		-- лианы: волнистые световые плети растут вверх, листья-блики
	...
	elseif shape == "Static" then
		-- статика: дёрганые узлы-помехи + дуга между случайными узлами
		for i = 1, n do
	...
	elseif shape == "Power Up" then
		-- пауэр-ап: столб энергии + искры стягиваются + кольца-заряды у ног
	...
	elseif shape == "Star Shower" then
		-- звёздопад: звёзды прочерчивают дуги над головой и падают вокруг
		for i = 1, math.max(3, math.min(n, 6)) do
	...
	elseif shape == "Ground Fog" then
		-- туман: стелющиеся клубы у земли, живое дыхание
		local fogs = {}
		for i = 1, math.max(3, math.min(n, 6)) do
		...
		-- сакура-дрейф: лепестки летят по ветру вокруг игрока
		for i = 1, n do
	...
	Tick.use("Aura", function(dt, t)
		...
		-- v5.1 ZHIVOST: свой темп/фаза у каждого узла (шум вместо ровного цикла)
		for i, fn in ipairs(movers) do
			fn(t + organicNoise(t * 0.8, i * 2.3) * 0.22 + i * 0.7, spd * (1 + 0.1 * organicNoise(t * 0.55, i * 5.1)))
		end
	end)
	do -- v5.2 ДЕТАЛИ: восходящие искры-моты поверх любой ауры
...
-- штамп на земле: одиночный плоский спрайт (VelocityPerpendicular), лежит и медленно гаснет
...
local function footprint(cf, pal, size, style, yaw)
	...
	if style == "Ring Step" then
		-- чистый шаг: аккуратное кольцо + микроблик
	...
	elseif style == "Paw Puff" then
		-- лапка: подушечка + четыре пальца + лёгкий пух
	...
	elseif style == "Frost Print" then
		-- морозный отпечаток: снежинка-кристалл + инеевый ободок
	...
	elseif style == "Ripple Pad" then
		-- круги по воде: два расходящихся кольца + блик глади
		for i = 1, 2 do
	...
	elseif style == "Star Stamp" then
		-- звёздный штамп: контур звезды + свечение
	...
	elseif style == "Ember Trail" then
		-- тлеющий след: выжженное пятно + искры-угли
	...
	elseif style == "Water Step" then
		-- водный шаг: круги + крохотный всплеск
	...
	elseif style == "Glyph Stamp" then
		-- глиф: сворл-руна + расходящееся кольцо-печать
...
function Pet.build()
	...
	if style == "Lantern Fish" then
		-- фонарь-рыба: корпус-оболочка, хвост-веер, фонарик-приманка со светом
	...
	elseif style == "Drone" then
		-- дрон: корпус, четыре ротора с размытием, сканер-луч вниз
	...
	elseif style == "Ghost" then
		-- призрак: простыня-складки, чёрные глаза, дым снизу, тайное мерцание
	...
	elseif style == "Dragonling" then
		-- дракончик: цепочка сегментов, перепончатые крылья, огненное дыхание
		local n = q(8, 5)
		local segs = {}
	...
	elseif style == "Cube" then
		-- куб: три ортогональных каркаса вращаются с разными скоростями, ядро-глитч
		for i = 1, 3 do
	...
	do -- v5.3 KONSTRUKTOR: куб собирается из 8 угловых деталей
	...
	elseif style == "Comet" then
		-- комета: ледяное ядро, огненный хвост-конус, искристый след
	...
	elseif style == "Bat" then
		-- летучая мышь: тело-капля + два перепончатых крыла, машут в противофазе
	...
	elseif style == "Hummer" then
		-- колибри: миниатюрное тело, размытые крылья-бумеранги, длинный клюв
	...
	elseif style == "Blob" then
		-- слайм: желе-полусфера дышит, капли стекают, блик-глаз
	...
	elseif style == "Sentinel" then
		-- страж: броневое кольцо-жалюзи + всевидящее око с зрачком
		for i = 1, 3 do
	...
	elseif style == "Nebula Ball" then
		-- туманность: вихрь-текстура внутри полупрозрачной сферы + звёздный дождь
	...
	elseif style == "Origami Bird" then
		-- оригами-птица: плоские грани-крылья с линиями сгиба, бумажный киль
		local wings = {}
		for side = -1, 1, 2 do
	...
	elseif style == "Lightning Core" then
		-- грозовое ядро: шар-плазма + дуги-разряды во все стороны + гул искр
	...
	elseif style == "Snowflake" then
		-- снежинка: 6 лучей с усиками-каркасом, вращение в двух плоскостях
		for arm = 0, 2 do
	...
	elseif style == "Wisp" then
		-- огонёк: пламя-слеза + тлеющий хвостик + редкие проблески
	...
	elseif style == "Star Sprite" then
		-- звёздочка: пятилучевая звезда-спрайт + два спутника на наклонных орбитах
		...
		-- дождевая тучка: слоистые клубы, моросит дождь, редкая вспышка-гроза
	...
	Tick.use("Pet", function(dt, t)
		...
		-- ориентация по направлению движения (в локальных координатах корня)
	...
	do -- v5.2 ДЕТАЛИ: искорки вокруг питомца
...
function Trails.build()
	...
	local function buildOn(parent, scaleW, tag)
		...
		if style == "Silk" then
			-- шёлк: гладкая лента с атласными кромками
		...
		elseif style == "Blaze" then
			-- пламя: огненная лента + белое ядро + угли из хвоста
		...
		elseif style == "Frostbite" then
			-- обморожение: ледяная лента + инеевые кромки-иглы + снежная пыль
		...
		elseif style == "Galaxy" then
			-- галактика: туманная тёмная лента + звёздное ядро + россыпь звёзд
		...
		elseif style == "Bubblegum" then
			-- жвачка: пухлые перемежающиеся ленты + мыльные пузыри
		...
		elseif style == "Umbra" then
			-- сумрак: чадная тёмная лента + светящаяся нить в сердце
		...
		elseif style == "Split Neon" then
			-- неоновый сплит: две цветные рельсы + белая нить по центру
		...
		elseif style == "Petals" then
			-- лепестки: широкая нежная лента + рой крутящихся лепестков
		...
		elseif style == "Datastream" then
			-- поток данных: пунктирные шины + сыплющиеся глифы + мерцание
			for i = -1, 1 do
		...
		elseif style == "Polar Light" then
			-- полярное сияние: высокий зыбкий занавес + бахрома-шиммер
		...
		elseif style == "Comet Tail" then
			-- кометный хвост: ядро-кокон + ореол + сноп искр назад
		...
		elseif style == "Static" then
			-- статика: рваная лента-помеха + искровые щелчки
		...
		elseif style == "Phantom" then
			-- фантом: широкая полупрозрачная мантия + тающая дымка
		...
		elseif style == "Spectrum" then
			-- спектр: радужная лента + призматические блики
		...
		elseif style == "Twin Helix" then
			-- двойная спираль: две ленты гуляют вверх-вниз в противофазе
		...
		elseif style == "Vapor" then
			-- пар: широкая дымчатая лента + тающие клубы
		...
		elseif style == "Stardust" then
			-- звёздная пыль: тонкая нить + рой звёздочек с вращением
		...
		elseif style == "Ember Wake" then
			-- угольный след: тлеющая лента + падающие угли + дымок
		...
		elseif style == "Stamp Path" then
			-- штампы: след из печатей-символов по пути (TextureMode.Static) + кольцо-подложка
			...
			-- вода: течения-ленты + брызги-капли + блики-росчерки
	...
	Tick.use("Trails", function(dt, t)
		...
		-- реакция на скорость: стоя — выключены, на бегу — длиннее
...
local function killFX(pos, victim)
	...
	if style == "Soul Column" then
		-- столб душ: световая колонна + восходящие виспы-души + кольца-волны
	...
	elseif style == "Supernova" then
		-- сверхновая: flipbook-взрыв + тройная волна + лучи-спицы по горизонту
	...
	elseif style == "Glass Shatter" then
		-- витрина разбита: паутина трещин + иглы-осколки + радужные блики
	...
	elseif style == "Inferno Pit" then
		-- адская пропасть: огненный обод + фонтаны лавы + чад + крылья жара
		local fire = cseq(WHITE, pal.bright, pal.base, tint(pal.deep, -0.5))
	...
	elseif style == "Singularity" then
		-- сингулярность: всё затягивается ВНУТРЬ, коллапс и белый выброс
		for i = 1, 2 do
	...
	elseif style == "Heart Pop" then
		-- сердце лопнуло: пульсирующий контур + рой мини-сердец + розовые волны
	...
	elseif style == "Deep Freeze" then
		-- вечная мерзлота: снежинка-кристалл + иглы льда + стынущий туман
	...
	elseif style == "Delete" then
		-- «удалён из сервера»: RGB-срезы + глифовый дождь + вертикальный скан
	...
	elseif style == "Rocket Salvo" then
		-- залп: три ракеты уходят вверх и распускаются фейерверком
		for i = 1, 3 do
	...
	elseif style == "Grave Bloom" then
		-- могила расцветает: крест из света + душа восходит + земля осыпается
	...
	elseif style == "Sky Strike" then
		-- орбитальный удар: луч с неба + громовая вспышка + раскол земли
	...
	elseif style == "Spirit Cage" then
		-- клетка духов: сходящиеся кольца + решётка-столбы + души внутри
		for i = 1, 4 do
	...
	elseif style == "Ink Execution" then
		-- казнь чернил: гигантская клякса + жгуты вверх + тёмный морок
	...
	elseif style == "Clockwork" then
		-- часовой механизм: циферблат, стрелки, зубчатые колёса, пар
	...
	elseif style == "Prism Cascade" then
		-- призматический каскад: белый раскол → веер спектра → радужный дождь
	...
	elseif style == "Shockwave Dome" then
		-- купол-ударная волна: двойной фронт + пылевое кольцо + дрожь земли
	...
	elseif style == "Detonation" then
		-- подрыв: кратер + слоистый взрыв + огненный столб + дымовое грибовидное облако
	...
	elseif style == "Star Burst" then
		-- звёздный взрыв: гигантская звезда-контур + рой звёзд + лучи
	...
	elseif style == "Smoke Screen" then
		-- дымовая завеса: плотные клубы по кругу + тлеющие угли внутри
		local n = q(8, 5)
		for i = 1, n do
			local a = i / n * TAU
		...
		-- волна: поднимающаяся стена воды по кругу + пена + брызги + волны-кольца
...
-- 1) AURA TRAILER — светящийся «шлейф-шлейф» (energy wake), тянущийся за
--    персонажем по направлению движения. Вместо тусклых линий к корню —
--    мягкие ленты-хвосты (Beam) + свечение + искры позади.
local AuraTrailer = defineModule("AuraTrailer", {
	Enabled  = false,
	Color    = Color3.fromRGB(120, 200, 255),
	Lifetime = 0.55,   -- плавность/длина хвоста
	Length   = 4,      -- длина следа в студах
	Width    = 1.1,    -- ширина лент
	Sparks   = true,
})
AuraTrailer.ribbons = {}
...
-- 2) FORCEFIELD — перекраска деталей рига в Material.ForceField.
--    (Детали не создаются — только перекрашиваются существующие; восстанавливаются при выключении.)
...
-- Каждому ассет-стилю — СВОЙ яркий процедурный «аварийный» образ, чтобы он
-- никогда не был пустым/одинаковым, даже пока ассет не догрузился.
...
-- Встроенные процедурные стили (местные текстуры, никакой сети). Значения —
-- размер/плотность/свечение подняты, чтобы было хорошо видно.
...
-- Применение процедурного (мгновенного) стиля — ярко и плотно.
local function applyBuiltin(style)
	...
	local function emitOn(part, scale)
		...
		-- v5.2 ДЕТАЛИ: слой-мерцание (крошечные белые вспышки поверх потока)
		local e2 = emitter(a, {
	...
	-- v5.2: плотность потоков «дышит» (органический шум, свой ритм у каждого)
...
-- Применение шаблонного (asset) стиля из кэша.
local function applyAsset(style, obj, color)
	-- v5.1: если кэш/GetObjects вернули не Instance — рисуем процедурный образ
...
function ParticleAura.build()
	...
	if ParticleAura.ids[style] then
		...
		-- ассет не готов: свой яркий процедурный образ у этого стиля
...
local MotionEcho = defineModule("MotionEcho", {
	...
	Lifetime = 0.75,
	Interval = 0.08,
	Transparency = 0.45,   -- чем меньше, тем плотнее/ярче копии
})
MotionEcho.live = {}
local lastPos, since, fadeAcc, totalParts = nil, 0, 0, 0
local cloneBusy = false
local HARD_CAP = 260   -- макс. суммарно «живых» частей-призраков
local SOFT_CAP = 180   -- выше этого уже не запускаем новый клон до сброса
...
-- кэшируем список деталей-«источников» между тиками (на 0.25с), чтобы не
-- гонять GetDescendants на КАЖДОМ кадре
...
-- снять «снимок» позы один раз (быстро), затем клонировать с задержками
local function makeEcho()
	...
	-- снимок: имя/класс/CFrame всех частей — фиксируем позу до начала клонирования
	local snap = {}
	...
	for i, s in ipairs(snap) do
		...
		-- размазать тяжёлую работу по кадрам (не грузим один кадр)
	...
	-- жёсткий сброс старых, если слишком много всего
...
-- настройки каждого стиля: текстура, размер, физика, поведение частицы
--   hue: каждая частица слегка сдвигает оттенок base-цвета (живой «перелив»)
--   twk: частота мерцания; grow: рост/усадка; fadeIn: мягкое проявление
ScreenFX.spec = {
...
-- настройки каждого стиля: текстура, размер в пикселях, скорость, жизнь, гравитация, растяжение
ScreenFX.spec = {
...
ScreenFX.panel = { gui = nil, frame = nil } -- состояние GUI (без лишних локальных переменных чанка)
...
function ScreenFX.spawnOne(spec, cfg, t)
	...
	img.Image = spec.tex
	img.ZIndex = 5
	-- цвет частицы: base-цвет с лёгким случайным сдвигом оттенка (живой перелив)
	local col = cfg.Color
	if spec.hue then
		local h, s, v = col:ToHSV()
		if s < 0.02 then h = math.random() end -- почти белый/чёрный → случайный оттенок
	...
	if spec.radial then
		-- линии скорости: рождаются у центра и летят наружу
...
function ScreenFX.step(dt, t)
	...
	for i = 1, #list do
		...
		if a >= 1 then
			...
			-- прозрачность: мягкое проявление → жизнь → быстрый уход
			local tr = a * a
			if spec.fadeIn and a < 0.2 then tr = (0.2 - a) * 2.5 end
			-- мерцание: живой «дыхательный» блеск
			if p.twk then
...
Sky.Presets = {
	...
	["Golden Evening"] = {
		-- рецепт из Creator Docs (enhance outdoor environments): ClockTime 17, лиловый Ambient, Atmosphere 0.272 / Haze 1
	...
	["Candlelit Indoor"] = {
		-- рецепт из Creator Docs (enhance indoor environments): тёплый ambient, плотная атмосфера, мягкие лучи
...
-- Skybox-наборы (порядок граней: Bk, Dn, Ft, Lf, Rt, Up). «Plain Sky» — встроенный
-- клиентский skybox, «Indoor Studio» — из официального туториала Creator Docs,
-- остальные — публичные наборы из каталога, собранные во время исследования.
...
-- Облака (Clouds под Terrain): Cover 0–1, Density 0.1–0.3 по рекомендациям docs
Sky.CloudSpecs = {
...
-- Фильтры поверх любого неба: ColorCorrectionEffect (+ Bloom для некоторых)
Sky.Filters = {
...
-- спрятать игровые Sky/Atmosphere/пост-эффекты (они могли быть возвращены режимом Default)
...
-- «Default» + выбранный skybox/облака: игровое освещение остаётся, меняется только купол/облака
local function applyDefaultExtras()
	...
	-- игровые объекты вернуть на место, кроме тех, что мы подменяем (небо / облака)
...
-- вращение skybox (SkyboxOrientation — новое свойство Sky; на старых клиентах тихо отключается)
Sky.spinAngle = 0
...
-- порывы ветра: workspace.GlobalWind = база + синусоиды (по docs); частицы с Drag и
-- WindAffectsDrag сносит, качается трава/листья Roblox-террейна
...
-- погода: 9 точек-эмиттеров (центр + сетка 3×3) следуют за камерой; у Attachment нет объёма,
-- поэтому осадки раскиданы по сетке. Дополнительные обработчики (гроза, сияние) — Sky.weather.extra
...
function Sky.setWeather(kind)
	...
	elseif w == "Aurora" then
		-- полярное сияние из Beam-занавесов высоко над камерой: слои с разной скоростью текстуры
		-- прячут повторение (совет из devforum), цвет зелёный → бирюзовый → фиолетовый
		local curtains = {}
		local colors = {
		...
		Sky.weather.extra = function(dt, t, p)
			for _, c in ipairs(curtains) do
				...
				-- X аттачмента → вдоль ленты, Y → вверх (ширина ленты = высота занавеса)
...
-- Current API: one tab, sections with subtitles, icon without .png.
PF_UI = {section=ODHX.shared.AddSection, notify=ODHX.shared.Notify}
-- A last-session snapshot is separate from the user's four manual profile slots.
if type(ODHX.data.snapshot)=="table" and type(ODHX.data.snapshot.settings)=="table" then
    ...
    -- Explicit manual-slot autoload retains its original meaning.
...
notify("PrismFlux Zero-Part v4 loaded ✦", 3)
...
-- ═══════════════════════════════════════════════════════════════════════════
-- ПЛАГИН: Shiftlock Crosshair   [shiftlock_color]   исходник: shiftlock_color.lua.txt
-- ═══════════════════════════════════════════════════════════════════════════
local function Plugin_shiftlock_color()
-- ODH 2026 adapter. Embedded in every plugin; no downloads/dependencies.
local ODHX = (function()
    local X = { ready=false, silent=false, restoring=false, replay=false, records={}, byKey={}, data={version=1, controls={}}, external=false }
    X.id, X.title, X.file = "shiftlock_color", "Shiftlock Crosshair", "ODH_shiftlock_color_settings.json"
    local host = ODH_HUB.For("shiftlock_color")
    assert(host and type(host.CreateTab)=="function", X.title .. ": load through the current Overdrive H plugin menu")
    local env = {}
    if type(getgenv)=="function" then local ok,g=pcall(getgenv); if ok and type(g)=="table" then env=g end end
    local rd = type(readfile)=="function" and readfile or env.readfile
...
-- ═══════════════════════════════════════════════════════════════════════════════════════════════════════════
-- 🎯 MOBILE SHIFTLOCK CROSSHAIR — ULTRA GLOW+ EDITION (v3.7)
-- Переработанная версия с улучшенными пульсациями и новыми функциями:

--   🌈 Rainbow/RGB режим (автономный HSV-цикл)
--   📐 Scale Pulse v2 — настраиваемая амплитуда + формы волны + центр масштаба
--   💗 Pulsate Transparency v2 — 4 формы волны (Sine/Breath/Triangle/Pulse) + min/max
--   👻 Trail/Echo — остаточные «призрачные» копии прицела (эффект следа)
--   💾 Автосохранение настроек (восстановление при перезагрузке)
--   🎲 Random Preset / Random Color
--   💾 Восстановление исходных свойств объектов при выключении

-- Совместима только с реальным API odh_shared_plugins:
--   shared.AddSection / shared.Notify
...
-- ═══════════════════════════════════════════════════════════════════════════════════════════════════════════
-- 🎨 ТАБЛИЦА ПРЕСЕТОВ ЦВЕТА
--   Каждый пресет = 5 ключевых точек для плавного циклического градиента.
--   Формат: { name = "Отображаемое имя", icon = "эмодзи", colors = { ... } }
-- ═══════════════════════════════════════════════════════════════════════════════════════════════════════════
...
-- Пресеты скорости анимации градиента
local SPEED_PRESETS = {
...
-- Пресеты скорости пульсации прозрачности/масштаба
local PULSE_PRESETS = {
...
-- 🆕 v3.5 Формы волны для пульсаций
local WAVEFORMS = {
...
-- 🆕 v3.5 Амплитуды для Scale Pulse
...
-- 🆕 v3.5 Глубина прозрачности для Pulsate
...
-- 🆕 v3.5 Количество echo-копий для Trail
...
-- Ключевые слова для поиска объектов прицела (на нескольких языках)
local CROSSHAIR_KEYWORDS = { "crosshair", "прицел", "aim", "reticle", "target", "cursor" }


-- 🆕 Ключ сохранения настроек
...
local state = {
    enabled = false,
    speed = SPEED_PRESETS[2].value,  -- Normal по умолчанию
    direction = 1,                   -- 1 = вперёд, -1 = реверс
    reversed = false,
    currentPreset = nil,             -- выбранный пресет (таблица)
    customColors = nil,              -- кастомные цвета из Colorpicker


-- 🌈 Rainbow
rainbowMode = false,             -- RGB/радуга (HSV-цикл)
rainbowSpeed = 1.0,              -- скорость радуги

-- 💗 Pulsate Transparency v2
pulsate = false,
pulseSpeed = 2,
pulseWaveform = "sine",          -- форма волны
pulseDepth = 0.4,                -- макс прозрачность (глубина)

-- 📐 Scale Pulse v2
scalePulse = false,
scaleAmplitude = 0.10,           -- на сколько «дышит» (10% по умолчанию)

-- 👻 Trail/Echo
trailEnabled = false,
trailCount = 2,                  -- количество призрачных копий
trailCopies = {},                -- созданные echo-объекты
trailBuilding = false,           -- 🛡️ guard: защита от повторного вызова buildTrail

targetObjects = {},              -- найденные объекты прицела
originalProps = {},              -- исходные свойства объектов для восстановления
rotation = 0,                    -- текущий угол вращения градиента
offset = 0,                      -- текущее смещение градиента
pulsePhase = 0,                  -- фаза пульсации
rainbowHue = 0,                  -- текущий оттенок радуги

renderConn = nil,                -- соединение RenderStepped
addedConn = nil,                 -- соединение DescendantAdded
removedConn = nil,               -- соединение DescendantRemoving
...
-- 🆕 Функции формы волны — возвращают значение 0..1 по фазе
local WAVEFORM_FUNCS = {
    -- 〰️ Плавная синусоида
    sine = function(phase)
        return math.sin(phase) * 0.5 + 0.5
    end,
    -- 💗 «Дыхание» — медленный вдох/выдох (синус в кубе для мягкости)
    breath = function(phase)
        local s = math.sin(phase) * 0.5 + 0.5
        return s * s * (3 - 2 * s)  -- smoothstep для плавности
    end,
    -- 📐 Треугольная — линейный подъём/спад
...
-- 🆕 Сохранение/загрузка настроек (через _G буфер)
...
local function saveSettings()
    _G[SETTINGS_KEY]=captureSettings() -- compatibility with an in-session older copy
    ODHX.Commit()
end
local function loadSettings()
...
-- Возвращает текущий активный набор цветов (кастомный или пресет)
local function getActiveColors()
    if state.rainbowMode then
        return nil  -- динамически строится в RenderStepped
    elseif state.customColors then
        return state.customColors
    elseif state.currentPreset then
        return state.currentPreset.colors
    end
    return GLOW_PRESETS[13].colors  -- запасной (Electric Purple)
end


-- Строит ColorSequence из массива цветов
...
-- Проверяет, является ли объект частью прицела по имени
local function isCrosshairObject(obj)
    if not (obj:IsA("ImageLabel") or obj:IsA("ImageButton")) then
        return false
    end
    local name = obj.Name:lower()
    -- 🛡️ Защита от бесконечной рекурсии: игнорируем trail-клоны
    -- (их имена начинаются с "crosshairtrail_"), иначе DescendantAdded
    -- сработает на клоне → buildTrail() → новые клоны → краш
...
-- Создаёт/возвращает UIGradient для объекта
...
-- Сохраняет исходные свойства объекта (для корректного восстановления)
local function saveOriginalProps(obj)
    ...
    table.insert(state.originalProps, {
        obj = obj,
...
-- Восстанавливает исходные свойства объекта
...
-- Применяет текущие цвета ко всем целевым объектам
...
-- Обновляет список объектов и применяет цвета к новым
...
-- ═══════════════════════════════════════════════════════════════════════════════════════════════════════════
-- 👻 TRAIL / ECHO — создание призрачных копий прицела
--   Каждая копия — клон исходного ImageLabel со смещением по позиции и повышенной прозрачностью,
--   обновляется каждый кадр (позиция/размер/цвет наследуются от оригинала с задержкой).
-- ═══════════════════════════════════════════════════════════════════════════════════════════════════════════
...
local function buildTrail()
    -- 🛡️ Защита от рекурсивного вызова (DescendantAdded может триггерить)
...
-- 🛡️ Жёсткий лимит: не больше 8 копий на объект (защита от краша)
...
for _, obj in ipairs(state.targetObjects) do
    for i = 1, maxPerObject do
        pcall(function()
            local clone = obj:Clone()
            -- уникальное имя (префикс CrosshairTrail_ исключается в isCrosshairObject)
            clone.Name = "CrosshairTrail_" .. i
            -- очищаем градиент оригинала — будет свой, создадим сразу
            local g = clone:FindFirstChild("CrosshairGradient")
            if g then g:Destroy() end
            -- создаём свой градиент СРАЗУ (не каждый кадр!)
            local cg = Instance.new("UIGradient")
            cg.Name = "CrosshairGradient"
            -- копируем цвет/ротацию из источника
            ...
            cg.Parent = clone
            -- повышенная прозрачность, дальше = прозрачнее
            clone.ImageTransparency = 0.3 + (i / (maxPerObject + 1)) * 0.5
            -- ZIndex ниже оригинала
            clone.ZIndex = (obj.ZIndex or 1) - i
            -- помещаем рядом (позицию задаём в RenderStepped)
...
-- Обновляет позиции echo-копий (вызывается из RenderStepped)
local function updateTrailPositions(scaleMul)
    local idx = 0
    for _, obj in ipairs(state.targetObjects) do
        -- находим запись с исходными свойствами
        local entry = nil
        ...
        if not entry then
            -- пропускаем (нет исходных данных)
        else
            for i = 1, state.trailCount do
                ...
                if copy and copy.Parent then
                    pcall(function()
                        -- смещение позиции — копии «тянется» в сторону движения масштаба
                        ...
                        -- позиция: копия немного смещается в зависимости от индекса
                        ...
                        -- 🛡️ синхронизируем цвет/градиент с оригиналом
                        -- БЕЗ создания новых Instance — только обновляем существующий
                        local srcGrad = obj:FindFirstChild("CrosshairGradient")
                        if srcGrad then
...
-- Удаляем градиенты и восстанавливаем исходные свойства
for _, obj in ipairs(state.targetObjects) do
    pcall(function()
        local grad = obj:FindFirstChild("CrosshairGradient")
        if grad then
...
state.renderConn = ODHX.Connect(RunService.RenderStepped, function(dt)
    ...
    -- Вращение/перетекание градиента
    ...
    -- Вычисляем значение пульсации (0..1) выбранной формой волны
    local waveVal = 0
    ...
    -- Пульсация прозрачностью (улучшенная)
    local transparency = 0
    if state.pulsate then
        -- базовая волна от 0 до pulseDepth
        transparency = waveVal * state.pulseDepth
    end

    -- Scale Pulse (улучшенный): амплитуда
    local scaleMul = 1
    ...
    for _, obj in ipairs(state.targetObjects) do
        pcall(function()
            ...
            if grad then
                ...
                grad.Rotation = state.rotation
                grad.Offset = Vector2.new(state.offset, 0)
            end

            -- прозрачность
            obj.ImageTransparency = transparency

            -- масштаб-пульсация (относительно исходного размера)
            if state.scalePulse then
                local entry = nil
                for _, e in ipairs(state.originalProps) do
                    if e.obj == obj then
...
if playerGui then
    ...
    state.removedConn = ODHX.Connect(playerGui.DescendantRemoving, function(descendant)
        -- 🛡️ Если удаляется trail-клон — не пересоздаём trail (иначе цикл)
...
-- ═══════════════════════════════════════════════════════════════════════════════════════════════════════════
-- 💗 ПУЛЬСАЦИЯ ПРОЗРАЧНОСТИ v2 (улучшенная)
-- ═══════════════════════════════════════════════════════════════════════════════════════════════════════════


shiftlock_section:AddLabel("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
shiftlock_section:AddLabel("💗 ПУЛЬСАЦИЯ v2 (улучшенная)")
shiftlock_section:AddLabel("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")


-- Тумблер пульсации прозрачностью
...
-- ═══════════════════════════════════════════════════════════════════════════════════════════════════════════
-- 👻 TRAIL / ECHO — призрачный след прицела
-- ═══════════════════════════════════════════════════════════════════════════════════════════════════════════


shiftlock_section:AddLabel("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
shiftlock_section:AddLabel("👻 TRAIL / ECHO (призрачный след)")
shiftlock_section:AddLabel("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
...
shiftlock_section:AddLabel("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
shiftlock_section:AddLabel("⚙️ НАСТРОЙКИ")
shiftlock_section:AddLabel("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
...
-- Обновить список прицелов вручную
...
shiftlock_section:AddLabel("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
shiftlock_section:AddLabel("✨ ПРЕСЕТЫ ЦВЕТА (" .. #GLOW_PRESETS .. ")")
shiftlock_section:AddLabel("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
...
shiftlock_section:AddLabel("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
shiftlock_section:AddLabel("📖 КАК РАБОТАЕТ:")
shiftlock_section:AddLabel("1. Включи 🔓 Enable Glow")
shiftlock_section:AddLabel("2. Выбери пресет / 🎨 Color / 🌈 Rainbow")
shiftlock_section:AddLabel("3. 💗 Пульсация: форма волны + глубина")
shiftlock_section:AddLabel("4. 📐 Scale Pulse: амплитуда дыхания")
shiftlock_section:AddLabel("5. 👻 Trail — призрачный след прицела")
shiftlock_section:AddLabel("6. 🎲 Random — для веселья!")
shiftlock_section:AddLabel("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
...
print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
print("🌈 Rainbow/RGB режим")
print("💗 Пульсация v2: 4 формы волны + глубина")
print("📐 Scale Pulse v2: настраиваемая амплитуда")
print("👻 Trail/Echo — призрачный след")
print("🎨 Colorpicker + " .. #GLOW_PRESETS .. " пресетов цвета")
print("🎲 Random Preset / Random Color")
print("💾 Автосохранение настроек")
print("🔍 Авто-обновление объектов прицела")
print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")


-- Запускаем восстановление сохранённых настроек
restoreSettings()
...
-- ═══════════════════════════════════════════════════════════════════════════
-- ПЛАГИН: Bindable Buttons Color   [BindableButtonsColor]   исходник: BindableButtonsColor.lua.txt
-- ═══════════════════════════════════════════════════════════════════════════
local function Plugin_BindableButtonsColor()
-- ODH 2026 adapter. Embedded in every plugin; no downloads/dependencies.
local ODHX = (function()
    local X = { ready=false, silent=false, restoring=false, replay=false, records={}, byKey={}, data={version=1, controls={}}, external=false }
    X.id, X.title, X.file = "BindableButtonsColor", "Bindable Buttons Color", "ODH_BindableButtonsColor_settings.json"
    local host = ODH_HUB.For("BindableButtonsColor")
    assert(host and type(host.CreateTab)=="function", X.title .. ": load through the current Overdrive H plugin menu")
    local env = {}
    if type(getgenv)=="function" then local ok,g=pcall(getgenv); if ok and type(g)=="table" then env=g end end
    local rd = type(readfile)=="function" and readfile or env.readfile
...
--[[ ============================================================
      Overdrive H  |  Plugin: Change Bindable Buttons Color (v5)
      ------------------------------------------------------------
      Changes the COLOR of bind buttons (Shoot Murderer etc.):

        • Four colorpickers (section:AddColorpicker):
            – button color    → BackgroundColor3 + ImageColor3
            – toggle ON color → what a toggled-ON button shows
            – text color      → TextColor3
            – stroke color    → UIStroke.Color + TextStrokeColor3

        • Toggle-state colors with CONTRAST flip, EVENT-DRIVEN:
          every watched button gets property-changed watchdogs
          (BackgroundColor3 / ImageColor3 / TextColor3 / UIStroke
          Color). When the GAME writes a color (its on/off
          indication), the plugin waits a short debounce, then
          classifies: back to the original = OFF, any other
          persistent color = ON. An ON button is shown in the
          CONTRAST color (dark button -> light ON color, light
          button -> dark ON color). One-shot game changes are no
          longer eaten by the repaint loop, and short press
          flashes (AutoButtonColor) are ignored.

        • The big on-screen "Shoot Murderer" HUD button (and any
          other wide panel) is NOT colored automatically anymore —
          auto-coloring only takes buttons narrower than ~150 px
          (or 18% of the screen width, whichever is larger).
          Want the big one colored? Pick it by tapping.

        • Save system (NEW in v3): a tap-picked button is only the
          CURRENT pick. Press "Save current button" to commit it —
          every SAVED button keeps its color forever (through new
          picks, rescans, toggle changes). Picking again replaces
          ONLY the unsaved pick, all saved buttons stay colored.
          "Clear saved buttons" restores them all at once.

        • Event-driven architecture (no periodic GUI-tree scans),
          duplicate-load protection, hard reset.

      The plugin changes NOTHING until you touch a colorpicker.
      After a reset, colors stay off until the next colorpicker
      change.

      Install: load this file in the Overdrive H menu as a plugin.
    ============================================================ ]]
...
local DEFAULT_BG         = Color3.fromRGB(255, 70, 70)   -- button color (background/icon)
local DEFAULT_ON_LIGHT   = Color3.fromRGB(120, 235, 200) -- ON color used when button color is DARK
local DEFAULT_ON_DARK    = Color3.fromRGB(28, 12, 48)    -- ON color used when button color is LIGHT
local DEFAULT_TEXT       = Color3.fromRGB(255, 255, 255) -- text color
local DEFAULT_STROKE     = Color3.fromRGB(0, 0, 0)       -- stroke color
local APPLY_INTERVAL     = 2   -- loop period (cheap cache repaint only)
local LIGHT_RESCAN_TICKS = 6   -- light hub-gui rescan every N ticks (~12 s)
local BIG_AUTO_MAX_WIDTH = 150 -- auto-color only buttons NARROWER than this
                               -- (px, or 18% of screen width if larger)
local PICK_SECONDS       = 6   -- how long we wait for the pick tap
local COMMIT_DELAY       = 0.35 -- debounce for game color writes (s)
...
pcall(function()
	if type(getgenv) == "function" then
		local g = getgenv()
		-- stop the previous copy if it supports Shutdown
...
local function genAlive()
	if MY_GEN == 0 then
		return true -- getgenv unavailable, no competitor exists
	end
	local ok, res = pcall(function()
...
--========================= PLUGIN STATE ==============================--
local State = {
	BgColor     = DEFAULT_BG,     -- button color
	OnLight     = DEFAULT_ON_LIGHT, -- ON color when button color is dark
	OnDark      = DEFAULT_ON_DARK,  -- ON color when button color is light
	TextColor   = DEFAULT_TEXT,   -- text color
	StrokeColor = DEFAULT_STROKE, -- stroke color
	Active      = false,          -- colorpicker touched? (false after reset)
	ManualBind  = nil,            -- current UNSAVED button picked by tap
	Saved       = {},             -- saved buttons (keep their color)
	Dead        = false,          -- instance unloaded by a newer copy
}
...
-- toggle-state tracking (weak tables, keyed by object):
--   weSet   — colors THIS plugin last assigned (lets us tell our
--             own repaints apart from changes made by the game)
--   pending — a game color write currently being observed
--             (debounced, ignores short press flashes)
--   btnMode — 1 = normal (off), 2 = the game repainted it (on)
--   SignalConns — per-object property-changed watchdogs
local weSet   = setmetatable({}, { __mode = "k" })
local pending = setmetatable({}, { __mode = "k" })
local btnMode = setmetatable({}, { __mode = "k" })
local SignalConns = setmetatable({}, { __mode = "k" })

-- cache of found buttons; painting works only on the cache — fast
local Cache = {
	list  = {},
	dirty = true,
	tick  = 0,
...
-- forward declarations (defined below, needed by addCandidate earlier)
...
-- strips RichText tags AND HTML entities
...
-- short safe name for notifications
...
-- are all ancestors visible? (called ONLY on matched candidates)
...
-- is the object inside the Overdrive H window/menu itself?
-- (only for matched candidates — not part of the hot path)
...
-- is the object inside the SYSTEM roblox GUI (RobloxGui etc.)?
-- such interfaces are never touched
...
-- converts anything into a Color3 or nil (a colorpicker callback
-- may arrive without arguments — then we keep the current color)
...
-- approximate Color3 equality (for toggle-state detection)
...
-- is the color dark? (decides which ON color to show)
local function isDarkColor(c)
	return luminance(c) < 0.45
end

--=============== FAST CANDIDATE FILTER (hot path) ====================
-- One pcall, minimal allocations. Recursive label lookup only for
-- buttons (their subtree is small). Returns true/false.
local function probeCandidate(inst)
	local ok, res = pcall(function()
		...
		-- recursive label search ONLY for buttons (small subtree)
		local childText = nil
		if isBtn then
...
-- is this too wide to be auto-colored? (the big on-screen
-- "Shoot Murderer" HUD button and other large panels);
-- tap-picked buttons bypass this check completely
...
local function addCandidate(inst)
	...
	-- if the mode is active — paint the new candidate right away
...
-- full scan (ONLY on load / first colorpicker touch)
local function fullScan()
	...
	-- merge with saved buttons (they survive every pick); dead
	-- ones are pruned from the saved list here
	local keptSaved = {}
...
-- light rescan: only ScreenGuis of the hub (not the whole CoreGui),
-- catches buttons whose text was set AFTER creation
...
--========================= REPAINT IN COLOR ==========================
-- Snapshot the ORIGINAL colors once per object, then simply assign
-- the colors chosen with the colorpickers. Also detects toggle
-- states (see TOGGLE DETECTION inside paint).

-- forward declarations (bodies are assigned a bit below)
...
-- Which color properties we watch for game writes (toggle
-- indication): kind name -> property name.
local WATCH_PROPS = {
...
-- topmost tracked ancestor (the cache candidate this object
-- belongs to) — the toggle state is stored on it as well
...
-- the color we normally set for this kind (mode 1)
...
local function classifyNow(obj, kind)
	...
	-- our own repaint landed meanwhile — not a game toggle
	if w[kind] and closeColors(now, w[kind]) then
		return
	end
	local mode = 2
	if snap[kind] and closeColors(now, snap[kind]) then
		mode = 1 -- back to the ORIGINAL game color = OFF
	end
	if closeColors(now, normalKindColor(kind)) then
		mode = 1 -- restored to OUR normal color = press blip
	end
	btnMode[obj] = mode
...
-- the GAME wrote a color: observe it for COMMIT_DELAY seconds;
-- if it survives (not a short press flash), classify the toggle
			...
			pcall(function()
				local conn = obj:GetPropertyChangedSignal(prop):Connect(function()
					...
					if closeColors(cur, w[kind]) then
						-- our own write echo — nothing to detect
						pending[obj] = nil
						return
					end
					-- the GAME wrote a color — observe it
					observe(obj, kind)
				end)
...
-- CONTRAST flip: dark button color -> light ON color,
-- light button color -> dark ON color
...
local function paint(obj)
	pcall(function()
		-- an observation is in progress (the game just wrote a
		-- color) — do NOT clobber it before it is classified
		if pending[obj] then
			return
		end
		...
		if mode == 2 then
			-- CONTRAST flip: dark button -> light ON color,
			-- light button -> dark ON color
			mainColor = onColor()
			strokeColor = onColor()
		end
		-- record what we are about to set BEFORE writing, so our
		-- own property-changed events recognize themselves
		local w = {}
...
-- FAST painting: cache only, NO scans. Called by the colorpickers
-- and the loop — costs almost nothing. This loop is also what
-- watches for toggle-state changes.
local function applyCached()
	...
	for _, inst in ipairs(Cache.list) do
		...
		if alive then
			...
			-- the button is gone: drop its color watchdogs
...
local function onDescendantAdded(inst)
	...
	-- O(1) check of ONLY the new object
	local okG, isGui = pcall(function()
		return inst:IsA("GuiObject")
	end)
...
--==================== MAINTENANCE LOOP (ECONOMY MODE) ================
-- Only a cheap cache repaint; a light hub-gui rescan runs every
-- LIGHT_RESCAN_TICKS ticks. NO full scans.
...
if not eventsOk then
	-- events unavailable: fallback loop with a full scan every 4 ticks
	pcall(function()
...
local function pickAt(x, y)
	...
	if best then
		-- saved buttons keep their color no matter what; only the
		-- UNSAVED pick is replaced (restored) by a new tap-pick
...
-- commits the current tap-picked button into the saved list;
-- saved buttons keep their color through new picks and rescans
...
-- removes the UNSAVED pick only (saved buttons stay colored)
...
-- restores and clears ALL saved buttons (and the unsaved pick)
...
-- unload this instance (called by a newer copy on reload)
...
-- debug access from the executor console (NOT via shared!)
pcall(function()
	if type(getgenv) == "function" then
...
section:AddParagraph("Change button colors",
	"The colorpickers paint the small Shoot Murderer bind buttons: " ..
...
section:AddColorpicker("Button color (background/icon)", State.BgColor, function(c)
	...
	State.Active = true
	-- synchronous full scan ONLY if the cache is empty and stale
	-- (after a reset). Normal color dragging does NO scans.
...
shared.Notify("Bindable Buttons Color v5 loaded! Section: Bindable Buttons Color", 3)

-- initial scan is delayed so plugin loading stays fast
later(1, function()
...
-- ═══════════════════════════════════════════════════════════════════════════
-- ПЛАГИН: Button Transparency   [ButtonTransparency]   исходник: ButtonTransparency.lua.txt
-- ═══════════════════════════════════════════════════════════════════════════
local function Plugin_ButtonTransparency()
-- ODH 2026 adapter. Embedded in every plugin; no downloads/dependencies.
local ODHX = (function()
    local X = { ready=false, silent=false, restoring=false, replay=false, records={}, byKey={}, data={version=1, controls={}}, external=false }
    X.id, X.title, X.file = "ButtonTransparency", "Button Transparency", "ODH_ButtonTransparency_settings.json"
    local host = ODH_HUB.For("ButtonTransparency")
    assert(host and type(host.CreateTab)=="function", X.title .. ": load through the current Overdrive H plugin menu")
    local env = {}
    if type(getgenv)=="function" then local ok,g=pcall(getgenv); if ok and type(g)=="table" then env=g end end
    local rd = type(readfile)=="function" and readfile or env.readfile
...
--[[ ============================================================
      Overdrive H  |  Плагин: Button Transparency (v6)
      ------------------------------------------------------------
      ГЛАВНОЕ в v6 — УБРАНЫ ПЕРИОДИЧЕСКИЕ ФРИЗЫ:
        • Полностью событийная архитектура: НЕТ регулярных
          сканов дерева интерфейса. Roblox сам сообщает о новых
          кнопках через событие DescendantAdded — плагин проверяет
          ТОЛЬКО новый объект (O(1)), а не всё дерево.
          Раньше каждые ~6 секунд сканировались тысячи объектов
          CoreGui + у КАЖДОГО узла запускался рекурсивный поиск
          надписи — это и давало фриз каждые 2-3 секунды.
        • Горячий фильтр переписан: один pcall на объект,
          сравнение ClassName по таблице, рекурсивный поиск
          надписи ТОЛЬКО у маленьких кнопок (а не у всех узлов).
        • Цикл обслуживания остался, но делает только дешёвую
          перекраску 2-3 кнопок из кэша; лёгкая проверка gui
          самого хаба — раз в ~24 секунды (не CoreGui целиком).
        • Защита от дублей: если плагин загружен повторно,
          старая копия выгружается (Shutdown), чтобы не молотили
          два цикла одновременно.
        • Вся функциональность сохранена: один ползунок
          "Button — прозрачность", выбор кнопки тапом, убрать
          ручной выбор, жёсткий сброс. Прозрачность аддитивная
          (цвет не меняется), 0 = как было, 100 = невидимо.

      ВАЖНО: после загрузки v6 перезайди в игру один раз —
      чтобы старая копия v5 (её цикл нельзя остановить снаружи)
      точно выгрузилась.

      Установка: как PluginExample — загрузить файл в меню
      Overdrive H как плагин.
    ============================================================ ]]

local shared = ODHX.shared
if not shared then
	warn("[Button Transparency] Запусти этот файл как плагин через меню Overdrive H!")
	return
end

--=============== НАСТРОЙКИ ПО УМОЛЧАНИЮ (можно менять) ===============--
local DEFAULT_VALUE      = 0   -- стартовая прозрачность, % (0 = как было)
local APPLY_INTERVAL     = 4   -- период цикла (только дешёвая перекраска кэша)
local LIGHT_RESCAN_TICKS = 6   -- лёгкий досмотр gui хаба раз в N тиков (~24 с)
local BIG_MIN_WIDTH      = 100 -- шире (px) = большая панель
local PICK_SECONDS       = 6   -- сколько секунд ждём тап выбора

--==================== ЗАЩИТА ОТ ДУБЛЕЙ ПЛАГИНА =======================--
local MY_GEN = 0
pcall(function()
	if type(getgenv) == "function" then
		local g = getgenv()
		-- останавливаем предыдущую копию, если она умеет Shutdown
...
local function genAlive()
	if MY_GEN == 0 then
		return true -- getgenv недоступен, некому конкурировать
	end
	local ok, res = pcall(function()
...
--========================= СОСТОЯНИЕ ПЛАГИНА =========================--
local State = {
	Value      = DEFAULT_VALUE, -- прозрачность, %
	Active     = false,         -- ползунок трогали? (после сброса = false)
	ManualBind = nil,           -- кнопка, выбранная тапом
	Dead       = false,         -- экземпляр выгружен новой копией
}
...
-- кэш найденных кнопок; покраска работает только по кэшу — быстро
local Cache = {
	list  = {},
	dirty = true,
	tick  = 0,
...
-- forward-объявления (определены ниже, нужны addCandidate раньше)
...
-- убирает RichText-теги И HTML-сущности
local function stripRich(s)
	s = tostring(s):gsub("<[^>]*>", "")
	s = s:gsub("&%a+;", " ")
	return s
...
-- короткое безопасное имя для уведомлений
local function sanitize(s)
	s = tostring(s):gsub("%c", ""):gsub("%s", "")
	if #s > 16 then
		s = s:sub(1, 16) .. "…"
...
-- все предки видимы? (вызывается ТОЛЬКО у совпавших кандидатов)
local function isVisible(inst)
	local cur = inst
	local depth = 0
	while cur and depth < 60 do
...
-- находится ли объект внутри окна/меню самого Overdrive H
-- (только для совпавших кандидатов — в горячем пути не участвует)
local function isInsideOverdriveMenu(inst)
	local cur = inst.Parent
	local depth = 0
	while cur and depth < 60 do
...
-- находится ли объект внутри СИСТЕМНОГО roblox-gui (RobloxGui и
-- т.п.) — такие интерфейсы не трогаем никогда
local function insideRobloxGui(inst)
	local cur = inst.Parent
	local depth = 0
	while cur and depth < 60 do
...
--================= БЫСТРЫЙ ФИЛЬТР КАНДИДАТА (горячий путь) ================--
-- Один pcall, минимум аллокаций. Рекурсивный поиск надписи —
-- только для кнопок (у них мало детей). Возвращает true/false.
local function probeCandidate(inst)
	local ok, res = pcall(function()
		local cls = inst.ClassName
		local isBtn  = BTN_CLASSES[cls] or false
		...
		-- рекурсивный поиск надписи ТОЛЬКО у кнопок (поддерево мало)
		local childText = nil
		if isBtn then
			local lbl = inst:FindFirstChildWhichIsA("TextLabel", true)
			if lbl then
...
-- мелкая кнопка с ключевым словом shoot/murderer/bind
...
local function addCandidate(inst)
	...
	table.insert(Cache.list, inst)
	-- если режим активен — сразу красим новый кандидат
	if State.Active then
...
-- полный скан (ТОЛЬКО при загрузке / первом включении ползунка)
local function fullScan()
	...
	-- объединяем с уже выбранным тапом
	for _, c in ipairs(Cache.list) do
...
-- лёгкий досмотр: только ScreenGui хаба (не весь CoreGui),
-- ловит кнопки, чей текст был установлен ПОСЛЕ создания
local function lightScan()
    local restored=ODHX.Resolve(State.ManualPath)
...
--====================== АДДИТИВНАЯ ПРОЗРАЧНОСТЬ ======================--
-- Итог = 1 - (1 - исходная) * (1 - заданная): цвет не меняется.
...
-- БЫСТРАЯ покраска: только кэш, БЕЗ сканов. Вызывается ползунком
-- и циклом — стоит копейки.
local function applyCached()
	if not State.Active then
		return 0
	end
...
local function onDescendantAdded(inst)
	...
	-- O(1)-проверка ТОЛЬКО нового объекта
	local okG, isGui = pcall(function()
		return inst:IsA("GuiObject")
	end)
	if not (okG and isGui) then
...
--==================== ЦИКЛ ОБСЛУЖИВАНИЯ (ЭКОНОМНЫЙ) ====================--
-- Только дешёвая перекраска кэша; лёгкий досмотр gui хаба —
-- раз в LIGHT_RESCAN_TICKS тиков. НИКАКИХ полных сканов.
local eventsOk = false
pcall(function()
	eventsOk = connectEvents()
end)
...
if not eventsOk then
	-- события недоступны: запасной цикл с полным сканом раз в 4 тика
	pcall(function()
		if type(spawn) == "function" and type(wait) == "function" then
			spawn(function()
				while not State.Dead and genAlive() do
...
local function pickAt(x, y)
	...
	if best then
		...
		shared.Notify("Выбрано: " .. nm .. " [" .. cls .. "] " .. w .. "x" .. h
			.. " — управляется ползунком", 4)
		return nm .. "[" .. cls .. "]"
	end
	shared.Notify("Под этим тапом не нашлось кнопки. Попробуй ещё раз.", 3)
	return nil
end
...
local function startPick()
	if pickActive then
		shared.Notify("Режим выбора уже включён — тапни по кнопке!", 2)
		return
	end
	pickActive = true
	local ok, err = pcall(function()
		...
		pcall(function()
			task.delay(PICK_SECONDS, function()
				if pickActive then
					shared.Notify("Выбор отменён: время вышло.", 2)
					finish(nil, nil)
				end
			end)
		end)
	...
	if not ok then
		pickActive = false
		shared.Notify("Не удалось включить выбор: " .. sanitize(tostring(err)), 3)
		return
	end
	shared.Notify("Тапни по кнопке в течение " .. PICK_SECONDS .. " сек!", 4)
end
...
local function clearManual()
	...
	Cache.list = kept
	shared.Notify("Ручной выбор убран" .. (mb and ", вид кнопки восстановлен." or "."), 2)
end
...
local function hardReset()
	...
	Cache.dirty      = true
	restoreAll()
	shared.Notify("Сброшено. Прозрачность не действует, пока не подвинешь ползунок.", 3)
	local passes = { 0.3, 0.8, 1.5, 3 }
	for i, sec in ipairs(passes) do
		later(sec, function()
			if not State.Active and not State.Dead then
...
-- выгрузка экземпляра (вызывается новой копией при перезагрузке)
local function shutdown()
	State.Dead = true
	disconnectEvents()
	restoreAll()
...
-- отладочный доступ для консоли экзекьютора (НЕ через shared!)
pcall(function()
	if type(getgenv) == "function" then
...
section:AddParagraph("Прозрачность кнопок",
	"Один ползунок для всех кнопок Shoot Murderer. 0 = как было, " ..
	"100 = невидимо, цвет не меняется. Если нужная кнопка не " ..
	"красится — выбери её тапом.")

section:AddSlider("Button — прозрачность", 0, 100, State.Value, function(v)
	State.Value = v
	State.Active = true
	-- синхронный полный скан ТОЛЬКО если кэш пуст и устарел
	-- (после сброса). При обычном перетаскивании сканов нет.
	if Cache.dirty and #Cache.list == 0 then
		Cache.dirty = false
		pcall(fullScan)
	end
...
section:AddButton("Выбрать кнопку тапом", function()
	startPick()
end)

section:AddButton("Убрать ручной выбор кнопки", function()
	clearManual()
end)

section:AddButton("Сбросить (вернуть как было)", function()
	hardReset()
end)

section:AddLabel("0 = как было • 100 = невидимая")

shared.Notify("Button Transparency v6 загружен! Раздел: Button Transparency", 3)

-- первичный скан отложенно, чтобы не тормозить загрузку плагина
later(1, function()
	if not State.Dead then
		pcall(fullScan)
		pcall(applyCached)
...
ODHX.Bind("Button Transparency", "Button — прозрачность", "Slider", function() return State.Value end)
ODHX.cleanup=shutdown
ODHX.Finish()
end
...
do
    local PLUGIN_BODIES = {
        Aimlock = Plugin_Aimlock,
        Anims = Plugin_Anims,
        BJP = Plugin_BJP,
        CFG = Plugin_CFG,
        Emotes = Plugin_Emotes,
        PM_VALEX = Plugin_PM_VALEX,
        ["Pm-Wallhop"] = Plugin_Pm_Wallhop,
        PrismFlux = Plugin_PrismFlux,
        shiftlock_color = Plugin_shiftlock_color,
        BindableButtonsColor = Plugin_BindableButtonsColor,
        ButtonTransparency = Plugin_ButtonTransparency,
    }
    for _, tag in ipairs(LOAD_ORDER) do
        local info = PLUGIN_INFO[tag]
        local body = PLUGIN_BODIES[tag]
        if info and body then
            ODH_HUB.Run(info, body)
        else
            warn("[PM2] неизвестный тег в LOAD_ORDER: " .. tostring(tag))
        end
    end
    ODH_HUB.Finish()
end
